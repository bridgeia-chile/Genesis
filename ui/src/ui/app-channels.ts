...
export async function handleWhatsAppStart(host: genesisApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: genesisApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: genesisApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: genesisApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: genesisApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: genesisApp): string {
...
function resolveGatewayHttpAuthHeader(host: genesisApp): string | null {
...
function buildGatewayHttpHeaders(host: genesisApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: genesisApp) {
...
export function handleNostrProfileToggleAdvanced(host: genesisApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: genesisApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: genesisApp) {
  const state = host.nostrProfileFormState;
