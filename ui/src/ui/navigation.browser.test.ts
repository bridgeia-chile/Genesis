...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/genesis/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/genesis");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/genesis/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__genesis_CONTROL_UI_BASE_PATH__ = "/genesis";
    const app = mountApp("/genesis/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/genesis");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/genesis/sessions");
  });
