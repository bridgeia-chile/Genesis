  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === 'connected') {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'genesis Browser Relay: relay reconnecting…',
      })
    }
  }
...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'genesis Browser Relay (click to attach/detach)',
      })
      continue
    }
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'genesis Browser Relay: attached (click to detach)',
      })
    } catch {
      setBadge(tabId, 'on')
    }
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'genesis Browser Relay: connecting to local relay…',
    })
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'genesis Browser Relay: relay not running (open options for setup)',
      })
...
function onDebuggerDetach(source, reason) {
  ...
  if (reason === 'target_closed') {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'genesis Browser Relay: re-attaching after navigation…',
    })
