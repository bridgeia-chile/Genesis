...
process.env.genesis_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime genesis modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: genesisConfig) => {
    ...
    resolveAccount: (cfg: genesisConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: genesisConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
