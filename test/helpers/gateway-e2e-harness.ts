...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `genesis-e2e-${name}-`));
  const configDir = path.join(homeDir, ".genesis");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          genesis_SKIP_PROVIDERS: "1",
          genesis_SKIP_GMAIL_WATCHER: "1",
          genesis_SKIP_CRON: "1",
          genesis_SKIP_BROWSER_CONTROL_SERVER: "1",
          genesis_SKIP_CANVAS_HOST: "1",
          genesis_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
