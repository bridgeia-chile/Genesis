...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    genesisHome: process.env.genesis_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("genesis_HOME", snapshot.genesisHome);
  restoreKey("genesis_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked genesis_HOME.
  delete process.env.genesis_HOME;
  process.env.genesis_STATE_DIR = path.join(base, ".genesis");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "genesis-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".genesis", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
