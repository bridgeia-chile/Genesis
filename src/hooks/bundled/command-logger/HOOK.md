...
2. **Appends to log file** - Writes a JSON line to `~/.genesis/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.genesis/logs/commands.log ~/.genesis/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/genesis`:
   ```
   /home/username/.genesis/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
