...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => genesisConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("genesis-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): genesisConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
