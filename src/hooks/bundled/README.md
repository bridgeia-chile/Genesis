...
This directory contains hooks that ship with genesis. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.bridgeia-chile/Genesis.json`:
...
2. Restart gateway: `pkill -9 -f 'genesis.*gateway' && pnpm genesis gateway`
3. Enable the hook: `genesis hooks enable my-hook`
