...
function createBootstrapExtraConfig(paths: string[]): genesisConfig {
  return {
    hooks: {
      internal: {
        entries: {
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("genesis-bootstrap-extra-");
  ...
  it("re-applies subagent bootstrap allowlist after extras are added", async () => {
    const tempDir = await makeTempWorkspace("genesis-bootstrap-extra-subagent-");
