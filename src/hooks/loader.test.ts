...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["genesis_BUNDLED_HOOKS_DIR"]);
    process.env.genesis_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
