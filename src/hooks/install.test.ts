...
const fixtureRoot = path.join(os.tmpdir(), `genesis-hook-install-${randomUUID()}`);
let tempDirIndex = 0;
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@genesis/test-hooks",
        version: "0.0.1",
        genesis: { hooks: ["./hooks/one-hook"] },
  ...
  it("rejects hook pack entries that traverse outside package directory", async () => {
    ...
    expect(result.error).toContain("genesis.hooks entry escapes package directory");
  });
  ...
  it("rejects hook pack entries that escape via symlink", async () => {
    ...
    expect(result.error).toContain("genesis.hooks entry resolves outside package directory");
  });
});
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@genesis/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@genesis/test-hooks@0.0.1",
    });
