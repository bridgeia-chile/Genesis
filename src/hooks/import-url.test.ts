...
describe("buildImportUrl", () => {
  ...
  it("returns bare URL for bundled hooks (no query string)", () => {
    const url = buildImportUrl(tmpFile, "genesis-bundled");
    expect(url).not.toContain("?t=");
    expect(url).toMatch(/^file:\/\//);
  });

  it("appends mtime-based cache buster for workspace hooks", () => {
    const url = buildImportUrl(tmpFile, "genesis-workspace");
  ...
  it("appends mtime-based cache buster for managed hooks", () => {
    const url = buildImportUrl(tmpFile, "genesis-managed");
    expect(url).toMatch(/\?t=[\d.]+&s=\d+/);
  });

  it("appends mtime-based cache buster for plugin hooks", () => {
    const url = buildImportUrl(tmpFile, "genesis-plugin");
    expect(url).toMatch(/\?t=[\d.]+&s=\d+/);
  });

  it("returns same URL for bundled hooks across calls (cacheable)", () => {
    const url1 = buildImportUrl(tmpFile, "genesis-bundled");
    const url2 = buildImportUrl(tmpFile, "genesis-bundled");
    expect(url1).toBe(url2);
  });

  it("returns same URL for workspace hooks when file is unchanged", () => {
    const url1 = buildImportUrl(tmpFile, "genesis-workspace");
    const url2 = buildImportUrl(tmpFile, "genesis-workspace");
    expect(url1).toBe(url2);
  });

  it("falls back to Date.now() when file does not exist", () => {
    const url = buildImportUrl("/nonexistent/handler.js", "genesis-workspace");
