...
`;
    ...
    expect(parsed.genesis.emoji).toBe("💾");
    expect(parsed.genesis.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.genesis.emoji).toBe("📝");
    expect(parsed.genesis.events).toEqual(["command"]);
    expect(parsed.genesis.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.genesis.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"genesis": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"genesis": {"events": ["test"]}}');
  });
...
describe("resolvegenesisMetadata", () => {
  it("extracts genesis metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvegenesisMetadata(frontmatter);
  ...
  it("returns undefined when genesis key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        genesis: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with genesis" },
            { id: "npm", kind: "npm", package: "@genesis/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@genesis/hook");
  });
...
`;
    ...
    const genesis = resolvegenesisMetadata(frontmatter);
    expect(genesis).toBeDefined();
    expect(genesis?.emoji).toBe("💾");
    expect(genesis?.events).toEqual(["command:new", "command:reset"]);
    expect(genesis?.requires?.config).toEqual(["workspace.dir"]);
    expect(genesis?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const genesis = resolvegenesisMetadata(frontmatter);
    expect(genesis?.emoji).toBe("disk");
    expect(genesis?.events).toEqual(["command:new"]);
  });
});
