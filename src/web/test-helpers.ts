...
const CONFIG_KEY = Symbol.for("genesis:testConfigMock");
...
vi.mock("@whiskeysockets/baileys", () => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("genesis:lastSocket")] =
    created.lastSocket;
  return created.mod;
});
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("genesis:lastSocket")];
  if (typeof getter === "function") {
