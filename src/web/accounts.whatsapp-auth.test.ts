...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["genesis_OAUTH_DIR"]);
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-oauth-"));
    process.env.genesis_OAUTH_DIR = tempOauthDir;
  });
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-wa-auth-"));
    try {
