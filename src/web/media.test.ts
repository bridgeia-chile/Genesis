...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredgenesisTmpDir(), "genesis-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub genesis_STATE_DIR.
    // Also keep it outside the genesis temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["genesis_STATE_DIR"]);
    process.env.genesis_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "genesis-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredgenesisTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default genesis state workspace and sandbox roots", async () => {
  ...
  it("rejects default genesis state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
