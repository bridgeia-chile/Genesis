...
describe("shared/frontmatter", () => {
  ...
  test("resolvegenesisManifestBlock parses JSON5 metadata and picks genesis block", () => {
    const frontmatter = {
      metadata: "{ genesis: { foo: 1, bar: 'baz' } }",
    };
    expect(resolvegenesisManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });
  ...
  test("resolvegenesisManifestBlock returns undefined for invalid input", () => {
    ...
    expect(
      resolvegenesisManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
