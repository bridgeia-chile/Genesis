...
const fixtureRoot = path.join(os.tmpdir(), `genesis-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.genesis_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "genesis.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.genesis_BUNDLED_PLUGINS_DIR = bundledDir;
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.genesis_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.genesis_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadgenesisPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadgenesisPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.genesis_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ genesis_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
