...
import type { genesisPluginService, genesisPluginServiceContext } from "./types.js";
...
function createRegistry(services: genesisPluginService[]) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: genesisPluginServiceContext[] = [];

    const serviceA: genesisPluginService = {
      id: "service-a",
      start: (ctx) => {
        starts.push("a");
        contexts.push(ctx);
    ...
    const serviceB: genesisPluginService = {
      id: "service-b",
      start: (ctx) => {
        starts.push("b");
        contexts.push(ctx);
    ...
    const serviceC: genesisPluginService = {
      id: "service-c",
      start: (ctx) => {
        starts.push("c");
        contexts.push(ctx);
