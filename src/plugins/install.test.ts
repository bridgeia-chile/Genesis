...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `genesis-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        genesis: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@genesis/zipper",
      version: "0.0.1",
      genesis: { extensions: ["./dist/index.js"] },
    }),
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.genesis/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without genesis.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@genesis/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("genesis.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.genesis).toBeUndefined();
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@genesis/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@genesis/voice-call@0.0.1",
    });
