...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("genesis_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  it("reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    const previousStateDir = process.env.genesis_STATE_DIR;
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.genesis_STATE_DIR = previousStateDir;
      }
