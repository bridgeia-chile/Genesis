...
const TEMP_MEDIA_PREFIX = "genesis-media-";
...
function createGroqAudioConfig(): genesisConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): genesisConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): genesisConfig {
  return {
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredgenesisTmpDir();
  ...
  it("auto-detects sherpa for audio when binary and model files are available", async () => {
    ...
    const cfg: genesisConfig = { tools: { media: { audio: {} } } };
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        genesis_AGENT_DIR: isolatedAgentDir,
