...
describe("infra parsing", () => {
  describe("diagnostic flags", () => {
    ...
    it("treats env true as wildcard", () => {
      const env = { genesis_DIAGNOSTICS: "1" } as NodeJS.ProcessEnv;
      expect(isDiagnosticFlagEnabled("anything.here", undefined, env)).toBe(true);
    });

    it("treats env false as disabled", () => {
      const env = { genesis_DIAGNOSTICS: "0" } as NodeJS.ProcessEnv;
  ...
  describe("isMainModule", () => {
    ...
    it("returns true for dist/entry.js when launched via genesis.mjs wrapper", () => {
      expect(
        isMainModule({
          currentFile: "/repo/dist/entry.js",
          argv: ["node", "/repo/genesis.mjs"],
          cwd: "/repo",
          env: {},
          wrapperEntryPairs: [{ wrapperBasename: "genesis.mjs", entryBasename: "entry.js" }],
        }),
      ).toBe(true);
    });
    ...
    it("returns false when running under PM2 but this module is imported", () => {
      expect(
        isMainModule({
          currentFile: "/repo/node_modules/genesis/dist/index.js",
