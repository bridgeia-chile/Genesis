...
import { POSIX_genesis_TMP_DIR, resolvePreferredgenesisTmpDir } from "./tmp-genesis-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredgenesisTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `genesis-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredgenesisTmpDir({
    accessSync,
...
describe("resolvePreferredgenesisTmpDir", () => {
  it("prefers /tmp/genesis when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_genesis_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/genesis when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_genesis_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/genesis when /tmp/genesis is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/genesis when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/genesis is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/genesis is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/genesis is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
