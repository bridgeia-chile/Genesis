...
export function triggergenesisRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.genesis_SYSTEMD_UNIT,
        process.env.genesis_PROFILE,
      );
  ...
  const label =
    process.env.genesis_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.genesis_PROFILE);
