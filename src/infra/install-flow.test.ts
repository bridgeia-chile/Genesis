...
describe("resolveExistingInstallPath", () => {
  ...
  beforeEach(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-install-flow-"));
  });
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "genesis-install-flow");
    ...
    expect(withTempDirSpy).toHaveBeenCalledWith("genesis-plugin-", expect.any(Function));
  ...
  it("returns extract failure when extraction throws", async () => {
    vi.spyOn(installSource, "withTempDir").mockImplementation(
      async (_prefix, fn) => await fn("/tmp/genesis-install-flow"),
    );
