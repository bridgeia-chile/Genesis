...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-update-"));
  });
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "genesis.mjs"), "export {};\n", "utf-8");
  });
  ...
  function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "genesis", version: "1.0.0" };
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "genesis.mjs")} doctor --non-interactive --fix`]:
        {
          stdout: "",
        },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "genesis", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "genesis", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g genesis@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g genesis@beta --no-fund --no-audit --loglevel=error",
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "genesis");
    const staleDir = path.join(nodeModules, ".genesis-stale");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g genesis@latest --no-fund --no-audit --loglevel=error") {
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "genesis");
      await seedGlobalPackageRoot(pkgRoot);

      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g genesis@latest",
        onInstall: async () => {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "genesis", version: "2.0.0" }),
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g genesis@latest")).toBe(true);
    });
  });

  it("rejects git roots that are not a genesis checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-genesis-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when genesis.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "genesis.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("genesis doctor entry");
  });
