...
import { resolvegenesisPackageRoot, resolvegenesisPackageRootSync } from "./genesis-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvegenesisPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "genesis" + dist/control-ui/index.html
