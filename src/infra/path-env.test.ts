...
let ensuregenesisCliOnPath: typeof import("./path-env.js").ensuregenesisCliOnPath;

describe("ensuregenesisCliOnPath", () => {
  const envKeys = [
    "PATH",
    "genesis_PATH_BOOTSTRAPPED",
    "genesis_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensuregenesisCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling genesis exists", () => {
    const tmp = abs("/tmp/genesis-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "genesis");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.genesis_PATH_BOOTSTRAPPED;

    ensuregenesisCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.genesis_PATH_BOOTSTRAPPED = "1";
    ensuregenesisCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/genesis-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "genesis");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/genesis-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "genesis");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "genesis");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/genesis-path/case-linuxbrew");
