...
import { resolvegenesisPackageRoot } from "./genesis-root.js";
...
export async function runGatewayUpdateCheck(params: {
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("genesis update")}`,
      );
