...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", () => {
    const cases = [
      { cfg: {} as genesisConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
...
describe("runHeartbeatOnce", () => {
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: genesisConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: genesisConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("genesis-hb");
