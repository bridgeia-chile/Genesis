...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.genesis_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.genesis_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set genesis_APNS_TEAM_ID and genesis_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.genesis_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.genesis_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.genesis_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set genesis_APNS_PRIVATE_KEY_P8 or genesis_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading genesis_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function creategenesisPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    genesis: creategenesisPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
