...
function candidateBinDirs(opts: EnsuregenesisPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `genesis` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "genesis");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.genesis_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "genesis"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `genesis` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensuregenesisCliOnPath(opts: EnsuregenesisPathOpts = {}) {
  if (isTruthyEnvValue(process.env.genesis_PATH_BOOTSTRAPPED)) {
    return;
  }
