...
describe("resolveEffectiveHomeDir", () => {
  it("prefers genesis_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands genesis_HOME when set to ~", () => {
    const env = {
      genesis_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for genesis_HOME", () => {
    const result = resolveRequiredHomeDir(
      { genesis_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when genesis_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ genesis_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { genesis_HOME: "/srv/genesis-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/genesis-home")}/x`);
  });
