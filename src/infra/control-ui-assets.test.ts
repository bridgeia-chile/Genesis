...
vi.mock("./genesis-root.js", () => ({
  resolvegenesisPackageRoot: vi.fn(async () => null),
  resolvegenesisPackageRootSync: vi.fn(() => null),
}));
...
let genesisRoot: typeof import("./genesis-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    genesisRoot = await import("./genesis-root.js");
  });
  ...
  it("uses resolvegenesisPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/genesis");
    (
      genesisRoot.resolvegenesisPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/genesis"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "genesis" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "genesis.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-genesis");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/genesis-bundle");
    (
      genesisRoot.resolvegenesisPackageRootSync as unknown as ReturnType<typeof vi.fn>
