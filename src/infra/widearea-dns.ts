...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.genesis_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*genesis-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "genesis";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "genesis");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "genesis-gw");
  ...
  records.push(`_genesis-gw._tcp IN PTR ${instanceLabel}._genesis-gw._tcp`);
  records.push(`${instanceLabel}._genesis-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._genesis-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; genesis-content-hash: ${contentHash}\n${contentBody}`;
}
