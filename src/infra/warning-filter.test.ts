...
const warningFilterKey = Symbol.for("genesis.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "genesis_TEST_WARNING" });
      await new Promise((resolve) => setImmediate(resolve));
      expect(
        seenWarnings.find((warning) => warning.code === "genesis_TEST_WARNING"),
      ).toBeDefined();
    } finally {
