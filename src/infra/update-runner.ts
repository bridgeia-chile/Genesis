...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "genesis.mjs");
    ...
    const doctorStep = await runStep(
      step("genesis doctor", doctorArgv, gitRoot, { genesis_UPDATE_IN_PROGRESS: "1" }),
    );
