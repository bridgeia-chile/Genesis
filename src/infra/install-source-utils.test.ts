...
const TEMP_DIR_PREFIX = "genesis-install-source-utils-";
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("genesis-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-genesis-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "genesis-plugin@1.2.3",
          name: "genesis-plugin",
          version: "1.2.3",
          filename: "genesis-plugin-1.2.3.tgz",
    ...
    const result = await runPack("genesis-plugin@1.2.3", cwd);

    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "genesis-plugin-1.2.3.tgz"),
      metadata: {
        name: "genesis-plugin",
        version: "1.2.3",
        resolvedSpec: "genesis-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "genesis-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("genesis-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    ...
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            id: "@genesis/plugin-demo@2.0.0",
            filename: "genesis-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@genesis/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "genesis-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@genesis/plugin-demo@2.0.0",
      },
    });
  });
