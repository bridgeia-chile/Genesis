...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["genesis_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    if (!loaded) {
      ({ resolvegenesisPackageRoot } = await import("./genesis-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolvegenesisPackageRoot).mockResolvedValue("/opt/genesis");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/genesis/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
