...
const genesis_REPO_URL = "https://github.com/bridgeia-chile/Genesis.git";
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolvegenesisPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", genesis_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `genesis_GIT_DIR points at a non-git directory: ${params.dir}. Set genesis_GIT_DIR to an empty folder or an genesis checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`genesis_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
