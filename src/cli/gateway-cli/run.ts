...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.genesis_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.genesis_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.genesis_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.genesis_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("genesis setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or genesis_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (
    ...
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or genesis_GATEWAY_TOKEN/genesis_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("genesis gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: genesis_GATEWAY_TOKEN env if set)",
    )
