...
describe("command-registry", () => {
  ...
  it("registers doctor placeholder for doctor primary command", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "genesis", "doctor"]);
  ...
  it("does not narrow to the primary command when help is requested", () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "genesis", "doctor", "--help"]);
  ...
  it("registers grouped core entry placeholders without duplicate command errors", async () => {
    const program = createProgram();
    registerCoreCliCommands(program, testProgramContext, ["node", "genesis", "vitest"]);
    program.exitOverride();
    await withProcessArgv(["node", "genesis", "status"], async () => {
      await program.parseAsync(["node", "genesis", "status"]);
    });
