...
beforeEach(() => {
  ...
  originalHideBanner = process.env.genesis_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.genesis_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.genesis_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("genesis");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "genesis", "status", "--debug"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "genesis", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "genesis", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "genesis", "--version"],
    });
  ...
  it("hides banner when genesis_HIDE_BANNER is truthy", async () => {
    process.env.genesis_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "genesis", "status"],
    });
