...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { genesis_GATEWAY_TOKEN: "service-token" },
    });
    service.restart.mockResolvedValue(undefined);
    vi.unstubAllEnvs();
    vi.stubEnv("genesis_GATEWAY_TOKEN", "");
  ...
  it("uses env-first token precedence when checking drift", async () => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { genesis_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("genesis_GATEWAY_TOKEN", "env-token");
