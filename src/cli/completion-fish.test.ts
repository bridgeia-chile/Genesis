...
describe("completion-fish helpers", () => {
  ...
  it("builds a subcommand completion line", () => {
    ...
    expect(line).toBe(
      `complete -c genesis -n "__fish_use_subcommand" -a "plugins" -d 'Manage Bob'\\''s plugins'\n`,
    );
  });
  ...
  it("builds option line with short and long flags", () => {
    ...
    expect(line).toBe(
      `complete -c genesis -n "__fish_use_subcommand" -s s -l shell -d 'Shell target'\n`,
    );
  });
  ...
  it("builds option line with long-only flags", () => {
    ...
    expect(line).toBe(
      `complete -c genesis -n "__fish_seen_subcommand_from completion" -l write-state -d 'Write cache'\n`,
    );
  });
});
