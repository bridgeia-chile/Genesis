...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("genesis-camera-snap-");
    expect(mediaPaths[1]).toContain("genesis-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /genesis-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /genesis-canvas-snapshot-.*\.png$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /genesis-camera-snap-front-.*\.jpg$/,
      },
      {
