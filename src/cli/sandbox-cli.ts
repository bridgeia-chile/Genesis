...
const SANDBOX_EXAMPLES = {
  main: [
    ["genesis sandbox list", "List all sandbox containers."],
    ["genesis sandbox list --browser", "List only browser containers."],
    ["genesis sandbox recreate --all", "Recreate all containers."],
    ["genesis sandbox recreate --session main", "Recreate a specific session."],
    ["genesis sandbox recreate --agent mybot", "Recreate agent containers."],
    ["genesis sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["genesis sandbox list", "List all sandbox containers."],
    ["genesis sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["genesis sandbox recreate --all", "Recreate all containers."],
    ["genesis sandbox recreate --session main", "Recreate a specific session."],
    ["genesis sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["genesis sandbox recreate --browser --all", "Recreate only browser containers."],
    ["genesis sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["genesis sandbox explain", "Show effective sandbox config."],
    ["genesis sandbox explain --session agent:main:main", "Explain a specific session."],
    ["genesis sandbox explain --agent work", "Explain an agent sandbox."],
    ["genesis sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.genesis.ai/cli/sandbox")}\n`,
    )
    .action(() => {
