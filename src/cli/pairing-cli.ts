...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "docs.genesis.ai/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!channelRaw || !resolvedCode) {
        throw new Error(
          `Usage: ${formatCliCommand("genesis pairing approve <channel> <code>")} (or: ${formatCliCommand("genesis pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("genesis pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
      const accountId = String(opts.account ?? "").trim();
