...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@genesis/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@genesis/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@genesis/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@genesis/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @genesis/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@genesis/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@genesis/plugin-alpha@1.2.3",
        installPath: "/tmp/genesis/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@genesis/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @genesis/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@genesis/plugin-alpha@latest",
      true,
      "/tmp/genesis/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@genesis/plugin-alpha@latest",
      installPath: "/tmp/genesis/extensions/alpha",
      version: "1.2.3",
