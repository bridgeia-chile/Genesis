...
import type { ConfigFileSnapshot, genesisConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `genesis config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: genesisConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: genesisConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: genesisConfig, config: genesisConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: genesisConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
