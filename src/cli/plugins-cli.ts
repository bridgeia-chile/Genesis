...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage genesis plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.genesis.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: genesisConfig = enableResult.config;
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: genesisConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.genesis.ai/plugin");
      lines.push("");
