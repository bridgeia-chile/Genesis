...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-commands-"));
...
function buildParams(commandBody: string, cfg: genesisConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/genesis-session-store.json" },
    } as genesisConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/genesis-agent-compact";
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as genesisConfig;
