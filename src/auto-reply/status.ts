...
type AgentDefaults = NonNullable<NonNullable<genesisConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.genesis/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 genesis ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: genesisConfig): string {
  const lines = ["ℹ️ Help", ""];
