...
export function isCommandEnabled(cfg: genesisConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: genesisConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as genesisConfig),
...
export function getCommandDetection(_cfg?: genesisConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: genesisConfig) {
