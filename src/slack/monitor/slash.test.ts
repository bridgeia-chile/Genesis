...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "genesis_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "genesis_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("genesis_cmdarg")).toBe(true);
    expect(testHarness.options.has("genesis_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("genesis_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("genesis_cmdarg");
    expect(blockId).toContain("genesis_cmdarg_ext:");
    const token = (blockId ?? "").slice("genesis_cmdarg_ext:".length);
