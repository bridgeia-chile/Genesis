 ...
 * Runs as part of `genesis doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: genesisConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("genesis config set agents.defaults.memorySearch.provider openai")}`,
          "",
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`genesis auth add --provider ${resolved.provider}`)}`,
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("genesis auth add --provider openai")}`,
