...
export async function statusCommand(
  ...
  runtime.log(theme.heading("genesis status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("genesis security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("genesis security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.genesis.ai/faq");
  runtime.log("Troubleshooting: https://docs.genesis.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("genesis status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("genesis logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("genesis status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("genesis gateway probe")}`);
  }
}
