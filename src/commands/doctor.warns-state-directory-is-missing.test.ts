...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.genesis_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when genesis_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.genesis_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
