...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under genesis_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genesis-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-oauth-sync-"));
    ...
    process.env.genesis_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-oauth-nosync-"));
    process.env.genesis_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside genesis_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-oauth-external-"));
    process.env.genesis_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* genesis_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to genesis_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("genesis-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
