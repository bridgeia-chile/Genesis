...
export const resolvegenesisPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadgenesisPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolvegenesisPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.genesis_STATE_DIR;
  originalUpdateInProgress = process.env.genesis_UPDATE_IN_PROGRESS;
  process.env.genesis_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.genesis_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
