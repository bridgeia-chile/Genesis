...
async function removeMacApp(runtime: RuntimeEnv, dryRun?: boolean) {
  ...
  await removePath("/Applications/genesis.app", runtime, {
    dryRun,
    label: "/Applications/genesis.app",
  });
}
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled<UninstallScope>({
      ...
      options: [
        ...
        { value: "state", label: "State + config", hint: "~/.genesis" },
        { value: "workspace", label: "Workspace", hint: "agent files" },
        {
          value: "app",
          label: "macOS app",
          hint: "/Applications/genesis.app",
        },
      ],
