...
export async function loadValidConfigOrThrow(): Promise<genesisConfig> {
...
export async function updateConfig(
  mutator: (cfg: genesisConfig) => genesisConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: genesisConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: genesisConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("genesis agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
