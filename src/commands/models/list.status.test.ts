...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvegenesisAgentDir: vi.fn().mockReturnValue("/tmp/genesis-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/genesis-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/genesis-agent/auth-profiles.json"),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/genesis-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvegenesisAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/genesis-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/genesis-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/genesis-agent-custom");
