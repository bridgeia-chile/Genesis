...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvegenesisAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("genesis models auth setup-token")}\` or \`${formatCliCommand("genesis configure")}\`.`
          : `Run \`${formatCliCommand("genesis configure")}\` or set an API key env var.`;
