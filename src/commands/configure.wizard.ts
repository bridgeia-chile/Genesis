...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.genesis_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.genesis_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "genesis update wizard" : "genesis configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: genesisConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("genesis doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.genesis_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.genesis_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.genesis_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.genesis_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.genesis_GATEWAY_TOKEN;
