...
function resolveSandboxDockerImage(cfg: genesisConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: genesisConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: genesisConfig, image: string): genesisConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: genesisConfig, image: string): genesisConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export function noteSandboxScopeWarnings(cfg: genesisConfig) {
