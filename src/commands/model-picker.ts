...
type PromptDefaultModelResult = { model?: string; config?: genesisConfig };
...
function resolveConfiguredModelRaw(cfg: genesisConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: genesisConfig): string[] {
...
export function applyPrimaryModel(cfg: genesisConfig, model: string): genesisConfig {
...
export function applyModelAllowlist(cfg: genesisConfig, models: string[]): genesisConfig {
  const defaults = cfg.agents?.defaults;
