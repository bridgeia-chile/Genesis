...
async function runRepair(cfg: genesisConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      genesis_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service genesis_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      genesis_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses genesis_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ genesis_GATEWAY_TOKEN: "env-token" }, async () => {
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing genesis gateway next.",
    );
  });
});
