...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".genesis"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg: genesisConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.genesis_OAUTH_DIR?.trim()) {
    return true;
  }
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".genesis");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.genesis out of git; it contains credentials and session history.",
