...
beforeEach(() => {
  envSnapshot = captureEnv(["genesis_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-auth-"));
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as genesisConfig,
      makePrompter(true),
    );
