...
function pruneAuthProfiles(
  cfg: genesisConfig,
  profileIds: Set<string>,
): { next: genesisConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("genesis models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("genesis models auth setup-token")} or ${formatCliCommand(
      "genesis configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "genesis models auth login --provider openai-codex",
    )} or ${formatCliCommand("genesis configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("genesis configure")}\` or \`${formatCliCommand("genesis onboard")}\`.`;
}
