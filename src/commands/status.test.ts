...
beforeAll(() => {
  envSnapshot = captureEnv(["genesis_PROFILE"]);
...
vi.mock("../infra/genesis-root.js", () => ({
  resolvegenesisPackageRoot: vi.fn().mockResolvedValue("/tmp/genesis"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/genesis/pnpm-lock.yaml",
      markerPath: "/tmp/genesis/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("genesis status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("genesis status --all") ||
          l.includes("genesis --profile isolated status --all") ||
          l.includes("genesis status --all") ||
          l.includes("genesis --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.genesis_GATEWAY_TOKEN = "abcd1234";
    try {
