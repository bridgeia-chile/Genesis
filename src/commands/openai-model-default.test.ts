...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: genesisConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: genesisConfig) => { changed: boolean; next: genesisConfig };
  defaultModel: string;
  overrideConfig: genesisConfig;
  alreadyDefaultConfig: genesisConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: genesisConfig) => config,
      applyDefaultConfig: (config: genesisConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: genesisConfig = { agents: { defaults: {} } };
