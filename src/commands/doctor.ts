...
import { resolvegenesisPackageRoot } from "../infra/genesis-root.js";
...
function resolveMode(cfg: genesisConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("genesis doctor");

  const root = await resolvegenesisPackageRoot({
  ...
  let cfg: genesisConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("genesis configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("genesis config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("genesis setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("genesis doctor --fix")}" to apply changes.`);
  }
