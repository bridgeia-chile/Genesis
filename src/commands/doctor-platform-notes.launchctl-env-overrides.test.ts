...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "genesis_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("genesis_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv genesis_GATEWAY_TOKEN");
    expect(message).not.toContain("genesis_GATEWAY_PASSWORD");
  });
