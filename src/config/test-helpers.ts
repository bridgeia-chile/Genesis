...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "genesis-config-" });
}

export async function writegenesisConfig(home: string, config: unknown): Promise<string> {
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writegenesisConfig(home, config);
