...
export function parsegenesisVersion(raw: string | null | undefined): genesisVersion | null {
  if (!raw) {
    return null;
  }
...
export function comparegenesisVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parsegenesisVersion(a);
  const parsedB = parsegenesisVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
