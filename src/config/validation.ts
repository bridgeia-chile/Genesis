...
export function validateConfigObjectRaw(
  ...
  const validated = genesisSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as genesisConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as genesisConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as genesisConfig,
  };
}
