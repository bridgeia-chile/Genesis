...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("genesis-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "genesis-sandbox-browser-global" },
      agentBrowser: { network: "genesis-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("genesis-sandbox-browser-agent");
  });
