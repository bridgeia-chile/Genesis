...
// Keep integration tests deterministic: never read a real genesis.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.genesis_SESSION_CACHE_TTL_MS;
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      ...
      process.env.genesis_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
