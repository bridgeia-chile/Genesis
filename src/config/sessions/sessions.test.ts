...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/genesis/agents/main/sessions";
  ...
  it("rejects absolute sessionFile paths outside known agent sessions dirs", () => {
    ...
    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/genesis/agents/work/not-sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
...
describe("session store lock (Promise chain mutex)", () => {
  ...
  beforeAll(async () => {
    lockFixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "genesis-lock-test-"));
  });
