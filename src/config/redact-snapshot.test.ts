...
import type { ConfigFileSnapshot } from "./types.genesis.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.genesis/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/genesis/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/genesis/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/genesis/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/genesis/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = genesisSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "genesisConfig";
    const hints = mapSensitivePaths(genesisSchema, "", {});
