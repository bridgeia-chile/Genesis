...
const resolveStorePath = vi.fn(() => "/tmp/genesis-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/genesis-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/genesis-discord-process-test-sessions.json" },
        channels: {
          discord: {
