...
describe("resolveTelegramAutoSelectFamilyDecision", () => {
  ...
  it("prefers env enable over env disable", () => {
    ...
    expect(decision).toEqual({
      value: true,
      source: "env:genesis_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
    });
  });

  it("uses env disable when set", () => {
    const decision = resolveTelegramAutoSelectFamilyDecision({
      env: { genesis_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      nodeMajor: 22,
    });
    expect(decision).toEqual({
      value: false,
      source: "env:genesis_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
    });
  });

  it("prefers env enable over config", () => {
    const decision = resolveTelegramAutoSelectFamilyDecision({
      env: { genesis_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
...
describe("resolveTelegramDnsResultOrderDecision", () => {
  it("uses env override when provided", () => {
    const decision = resolveTelegramDnsResultOrderDecision({
      env: { genesis_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
    });
    expect(decision).toEqual({
      value: "verbatim",
      source: "env:genesis_TELEGRAM_DNS_RESULT_ORDER",
    });
  });
