...
function cfg(accounts?: Record<string, unknown> | null): genesisConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as genesisConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as genesisConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as genesisConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as genesisConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as genesisConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as genesisConfig)).toBe("default");
    });
  });
});
