...
describe("message action capability checks", () => {
  ...
  it("aggregates buttons/card support across plugins", () => {
    ...
    expect(supportsChannelMessageButtons({} as genesisConfig)).toBe(true);
    expect(supportsChannelMessageCards({} as genesisConfig)).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as genesisConfig, channel: "discord" }),
    ).toBe(true);
    expect(
      supportsChannelMessageButtonsForChannel({ cfg: {} as genesisConfig, channel: "telegram" }),
    ).toBe(false);
    expect(
      supportsChannelMessageCardsForChannel({ cfg: {} as genesisConfig, channel: "telegram" }),
    ).toBe(true);
    expect(supportsChannelMessageCardsForChannel({ cfg: {} as genesisConfig })).toBe(false);
  });
});
