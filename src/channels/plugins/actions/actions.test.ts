...
function telegramCfg(): genesisConfig {
  return { channels: { telegram: { botToken: "tok" } } } as genesisConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: genesisConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as genesisConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as genesisConfig;
...
function createSignalAccountOverrideCfg(): genesisConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as genesisConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as genesisConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as genesisConfig,
        accountId: undefined,
        params: {
