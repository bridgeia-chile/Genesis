...
function setWhatsAppDmPolicy(cfg: genesisConfig, dmPolicy: DmPolicy): genesisConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: genesisConfig, allowFrom?: string[]): genesisConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: genesisConfig, selfChatMode: boolean): genesisConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: genesisConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so genesis can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppAllowFrom(
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for genesis" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("genesis channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
