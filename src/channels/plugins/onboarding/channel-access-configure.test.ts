...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: genesisConfig, policy: ChannelAccessPolicy) => genesisConfig;
  resolveAllowlist: (params: { cfg: genesisConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: genesisConfig; resolved: TResolved }) => genesisConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: genesisConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: genesisConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: genesisConfig, policy: ChannelAccessPolicy): genesisConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: genesisConfig, policy: ChannelAccessPolicy): genesisConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: genesisConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: genesisConfig; resolved: string[] }) => {
      calls.push("apply");
