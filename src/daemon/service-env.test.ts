...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.genesis_GATEWAY_PORT).toBe("18789");
    expect(env.genesis_GATEWAY_TOKEN).toBe("secret");
    expect(env.genesis_SERVICE_MARKER).toBe("genesis");
    expect(env.genesis_SERVICE_KIND).toBe("gateway");
    expect(typeof env.genesis_SERVICE_VERSION).toBe("string");
    expect(env.genesis_SYSTEMD_UNIT).toBe("genesis-gateway.service");
    if (process.platform === "darwin") {
      expect(env.genesis_LAUNCHD_LABEL).toBe("ai.genesis.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", genesis_PROFILE: "work" },
      port: 18789,
    });
    expect(env.genesis_SYSTEMD_UNIT).toBe("genesis-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.genesis_LAUNCHD_LABEL).toBe("ai.genesis.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genesis"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", genesis_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genesis-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", genesis_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".genesis"));
  });

  it("uses genesis_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", genesis_STATE_DIR: "/var/lib/genesis" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/genesis"));
  });

  it("expands ~ in genesis_STATE_DIR", () => {
    const env = { HOME: "/Users/test", genesis_STATE_DIR: "~/genesis-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/genesis-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { genesis_STATE_DIR: "C:\\State\\genesis" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\genesis");
  });
});
