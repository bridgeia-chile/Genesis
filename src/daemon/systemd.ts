...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.genesis_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.genesis_PROFILE);
}
...
export async function installSystemdService({
  ...
  const serviceName = resolveGatewaySystemdServiceName(env.genesis_PROFILE);
