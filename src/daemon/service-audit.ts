...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.genesis_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service genesis_GATEWAY_TOKEN does not match gateway.auth.token in genesis.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `genesis gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
