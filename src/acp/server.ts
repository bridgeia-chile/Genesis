...
function printHelp(): void {
  console.log(`Usage: genesis acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or genesis_GATEWAY_TOKEN.",
    );
  }
  if (argv.includes("--password") || argv.includes("--gateway-password")) {
    console.error(
      "Warning: --password can be exposed via process listings. Prefer --password-file or genesis_GATEWAY_PASSWORD.",
    );
  }
