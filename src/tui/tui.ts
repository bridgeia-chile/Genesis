...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      pairingHint:
        "Pairing required. Run `genesis devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `genesis tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
