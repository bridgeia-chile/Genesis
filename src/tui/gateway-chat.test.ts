...
describe("resolveGatewayConnection", () => {
  ...
  it("uses genesis_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ genesis_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers genesis_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ genesis_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
