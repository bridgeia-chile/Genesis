...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("genesis-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("genesis-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("genesis-lid-a-", (first) => {
      withTempDirSync("genesis-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.genesis when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "genesis-config-dir-"));
    try {
...
describe("shortenHomePath", () => {
  it("uses $genesis_HOME prefix when genesis_HOME is set", () => {
    ...
    expect(shortenHomePath(`${path.resolve("/srv/genesis-home")}/.bridgeia-chile/Genesis.json`)).toBe(
      "$genesis_HOME/.bridgeia-chile/Genesis.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $genesis_HOME replacement when genesis_HOME is set", () => {
    ...
    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/genesis-home")}/.bridgeia-chile/Genesis.json`),
    ).toBe("config: $genesis_HOME/.bridgeia-chile/Genesis.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/genesis")).toBe(path.resolve(os.homedir(), "genesis"));
  });
