...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses genesis_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      genesis_HOME: "/srv/genesis-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/genesis-home"), ".genesis", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".genesis", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("genesis-workspace-");
