...
const fixtureSuite = createFixtureSuite("genesis-skills-prompt-suite-");
...
describe("buildWorkspaceSkillsPrompt", () => {
  ...
  it("gates by bins, config, and always", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"genesis":{"requires":{"bins":["fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "anybin-skill"),
      name: "anybin-skill",
      description: "Needs any bin",
      metadata: '{"genesis":{"requires":{"anyBins":["missingbin","fakebin"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "config-skill"),
      name: "config-skill",
      description: "Needs config",
      metadata: '{"genesis":{"requires":{"config":["browser.enabled"]}}}',
    });
    await writeSkill({
      dir: path.join(skillsDir, "always-skill"),
      name: "always-skill",
      description: "Always on",
      metadata: '{"genesis":{"always":true,"requires":{"env":["MISSING"]}}}',
    });
    await writeSkill({
  ...
  it("uses skillKey for config lookups", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"genesis":{"skillKey":"alias"}}',
    });
