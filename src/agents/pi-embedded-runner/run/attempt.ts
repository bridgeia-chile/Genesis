...
import { creategenesisCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
  ...
  try {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolvegenesisDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/bridgeia-chile/Genesis/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt") {
          try {
            sessionManager.appendCustomEntry("genesis:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/bridgeia-chile/Genesis/issues/8643
