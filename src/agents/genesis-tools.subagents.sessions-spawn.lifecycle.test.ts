...
} from "./genesis-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.genesis_TEST_FAST;
...
describe("genesis-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.genesis_TEST_FAST;
      return;
    }
    process.env.genesis_TEST_FAST = fastModeEnv.previous;
  });
