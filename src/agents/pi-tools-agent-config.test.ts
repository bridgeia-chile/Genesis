...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: genesisConfig) {
    return creategenesisCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<genesisConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<genesisConfig["agents"]>["list"]>[number]["tools"];
  }): genesisConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = creategenesisCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = creategenesisCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = creategenesisCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = creategenesisCodingTools({
      config: cfg,
    ...
    const bobTools = creategenesisCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = creategenesisCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = creategenesisCodingTools({
      config: cfg,
