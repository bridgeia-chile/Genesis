...
type CreategenesisTools = (typeof import("./genesis-tools.js"))["creategenesisTools"];
export type CreategenesisToolsOpts = Parameters<CreategenesisTools>[0];
...
export async function getSessionsSpawnTool(opts: CreategenesisToolsOpts) {
  ...
  const tool = creategenesisTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
