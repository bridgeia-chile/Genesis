...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): genesisConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as genesisConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): genesisConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as genesisConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: genesisConfig, accountId = "media") {
    await handleTelegramAction(
