...
const GOOGLE_LIVE_SINGLE_KEY = "genesis_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "genesis_LIVE_ANTHROPIC_KEY",
    listVar: "genesis_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "genesis_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `genesis_LIVE_${base}_KEY`;
