...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: genesisConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/genesis" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: genesisConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: genesisConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: genesisConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: genesisConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: genesisConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/genesis");
  });

  it("uses genesis_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "genesis-home");
    vi.stubEnv("genesis_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as genesisConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".genesis", "workspace"));
  });

  it("uses genesis_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "genesis-home");
    vi.stubEnv("genesis_HOME", home);
    // Clear state dir so it falls back to genesis_HOME
    vi.stubEnv("genesis_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as genesisConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".genesis", "agents", "main", "agent"));
  });
});
