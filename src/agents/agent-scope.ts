...
type AgentEntry = NonNullable<NonNullable<genesisConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: genesisConfig): AgentEntry[] {
...
export function listAgentIds(cfg: genesisConfig): string[] {
...
export function resolveDefaultAgentId(cfg: genesisConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: genesisConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: genesisConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: genesisConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: genesisConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
