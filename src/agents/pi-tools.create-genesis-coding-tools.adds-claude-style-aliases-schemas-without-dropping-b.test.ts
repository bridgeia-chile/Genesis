...
const defaultTools = creategenesisCodingTools({ senderIsOwner: true });
...
describe("creategenesisCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = creategenesisCodingTools({
      config,
    ...
    const anthropicTools = creategenesisCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = creategenesisCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = creategenesisCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
