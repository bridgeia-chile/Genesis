...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("genesis-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("genesis-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("genesis-sbx-");
