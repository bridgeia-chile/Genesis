...
describe("workspace bootstrap file caching", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("genesis-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("genesis-cache-test1-");
    const workspace2 = await makeTempWorkspace("genesis-cache-test2-");
