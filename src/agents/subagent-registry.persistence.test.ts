...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-subagent-"));
  ...
  it("uses isolated temp state when genesis_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "genesis-test-state"));
  });
});
