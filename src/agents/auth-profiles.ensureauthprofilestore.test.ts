...
describe("ensureAuthProfileStore", () => {
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "genesis-auth-merge-"));
    const previousAgentDir = process.env.genesis_AGENT_DIR;
    ...
    try {
      ...
      process.env.genesis_AGENT_DIR = mainDir;
      ...
      if (previousAgentDir === undefined) {
        delete process.env.genesis_AGENT_DIR;
      } else {
        process.env.genesis_AGENT_DIR = previousAgentDir;
      }
