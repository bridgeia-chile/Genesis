...
function resolveSkillsLimits(config?: genesisConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "genesis-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "genesis-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolvegenesisMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: genesisConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`genesis skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
