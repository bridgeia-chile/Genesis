...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["genesis_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns genesis_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-bundled-override-"));
    process.env.genesis_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.genesis_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "genesis" }));
