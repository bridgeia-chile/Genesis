...
describe("creategenesisCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-alias-"));
    try {
      const tools = creategenesisCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-structured-write-"));
    try {
      const tools = creategenesisCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-structured-edit-"));
    try {
