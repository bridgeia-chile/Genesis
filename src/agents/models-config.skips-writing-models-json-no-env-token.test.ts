...
async function runEnvProviderCase(params: {
  ...
  try {
    await ensuregenesisModelsJson({});

    const modelPath = path.join(resolvegenesisAgentDir(), "models.json");
...
describe("models-config", () => {
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      await withTempEnv([...MODELS_CONFIG_IMPLICIT_ENV_VARS, "KIMI_API_KEY"], async () => {
        ...
        process.env.genesis_AGENT_DIR = agentDir;
        process.env.PI_CODING_AGENT_DIR = agentDir;

        const result = await ensuregenesisModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      await ensuregenesisModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
