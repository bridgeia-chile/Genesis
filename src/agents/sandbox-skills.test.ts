...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "genesis-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.genesis_BUNDLED_SKILLS_DIR = bundledDir;
