...
describe("resolvegenesisAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          genesis_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolvegenesisAgentDir();
  ...
  it("honors genesis_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          genesis_STATE_DIR: undefined,
          genesis_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolvegenesisAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when genesis_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers genesis_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          genesis_STATE_DIR: undefined,
          genesis_AGENT_DIR: primaryOverride,
