...
async function listSandboxBrowserContainers(
  ...
  try {
    const result = await execDockerRawFn(
      ["ps", "-a", "--filter", "label=genesis.sandboxBrowser=1", "--format", "{{.Names}}"],
      { allowFailure: true },
    );
    if (result.code !== 0) {
      return null;
  ...
  try {
    const result = await params.execDockerRawFn(
      [
        "inspect",
        "-f",
        '{{ index .Config.Labels "genesis.configHash" }}\t{{ index .Config.Labels "genesis.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("genesis sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected genesis.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
      remediation: `${formatCliCommand("genesis sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("genesis sandbox recreate --browser --all")} (add --force to skip prompt), ` +
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = isToolAllowedByPolicies("__genesis_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `genesis plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `genesis hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `genesis security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your genesis extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "genesis-bundled") {
        continue;
      }
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `genesis security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
