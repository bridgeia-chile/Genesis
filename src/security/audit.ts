...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("genesis security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: genesisConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: genesisConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: genesisConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
