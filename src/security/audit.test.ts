...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: genesisConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ genesis_STATE_DIR: tmp }, async () => await fn(tmp));
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.genesis_GATEWAY_TOKEN;
    const prevPassword = process.env.genesis_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.genesis_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.genesis_GATEWAY_PASSWORD;
      } else {
        process.env.genesis_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "genesis.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("genesis-sbx-browser-old\ngenesis-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "genesis-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "genesis-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("genesis-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("genesis-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "genesis-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "genesis-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-genesis.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): genesisConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: genesisConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withStateDir("discord-name-based-allowlist", async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.genesis/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: genesisConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.genesis_GATEWAY_TOKEN;
    process.env.genesis_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: genesisConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<genesisConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: genesisConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.genesis",
      configPath: "/Users/test/Dropbox/.bridgeia-chile/Genesis.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: genesisConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "genesis.json"),
      });
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@genesis/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@genesis/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        genesis: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    beforeEach(() => {
      envSnapshot = captureEnv(["genesis_GATEWAY_TOKEN", "genesis_GATEWAY_PASSWORD"]);
      delete process.env.genesis_GATEWAY_TOKEN;
      delete process.env.genesis_GATEWAY_PASSWORD;
    });
    ...
    const setProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        process.env.genesis_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        process.env.genesis_GATEWAY_PASSWORD = env.password;
      }
    };
