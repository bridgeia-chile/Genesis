...
describe("canvas host state dir defaults", () => {
  it("uses genesis_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("genesis-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("genesis Canvas");
      } finally {
        await handler.close();
      }
    });
