...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("genesis-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.genesis_STATE_DIR).toBe(path.join(tempHome.home, ".genesis"));
    ...
    expect(process.env.genesis_STATE_DIR).toBe(previousStateDir);
