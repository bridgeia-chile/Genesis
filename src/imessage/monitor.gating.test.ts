...
function baseCfg(): genesisConfig {
  return {
    channels: {
      imessage: {
        dmPolicy: "open",
...
function buildDispatchContextPayload(params: { cfg: genesisConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@genesis hello",
      bodyText: "@genesis hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
