...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "genesis.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "genesis.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.genesis_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.genesis_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.genesis_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[genesis] ${parsed.error}`);
      process.exit(2);
    }
