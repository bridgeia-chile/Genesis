...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredgenesisTmpDir: vi.fn(() => "/tmp/genesis"),
}));
vi.mock("../infra/tmp-genesis-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredgenesisTmpDir.mockReturnValue("/tmp/genesis");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredgenesisTmpDir.mockReturnValue("/tmp/genesis-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "genesis-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredgenesisTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "genesis-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "genesis-preferred", "downloads")}${path.sep}`),
    );
  });
