...
const RELAY_TOKEN_CONTEXT = "genesis-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const genesis_RELAY_BROWSER = "genesis/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.genesis_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or genesis_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedgenesisRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === genesis_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
