...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.genesis_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/genesis" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          genesis: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchgenesisChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.genesis_GATEWAY_PORT;
  process.env.genesis_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.genesis_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.genesis_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.genesis_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.genesis_GATEWAY_PASSWORD;
  } else {
    process.env.genesis_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
