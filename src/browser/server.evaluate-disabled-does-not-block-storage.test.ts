  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          genesis: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.genesis_GATEWAY_PORT;
    process.env.genesis_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.genesis_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.genesis_GATEWAY_PASSWORD;
