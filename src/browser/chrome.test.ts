...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "genesis-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorategenesisProfile(userDataDir, { color: DEFAULT_genesis_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_genesis_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_genesis_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".genesis-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorategenesisProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_genesis_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopgenesisChrome no-ops when process is already killed", async () => {
    const proc = makeProc({ killed: true });
    await stopgenesisChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopgenesisChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopgenesisChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopgenesisChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
