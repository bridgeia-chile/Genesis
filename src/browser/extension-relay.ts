...
const RELAY_AUTH_HEADER = "x-genesis-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/genesis-Extension-Relay",
          revision: "0",
          userAgent: "genesis-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "genesis/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedgenesisRelay({
