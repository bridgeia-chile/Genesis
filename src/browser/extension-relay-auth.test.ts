...
import {
  probeAuthenticatedgenesisRelay,
...
describe("extension-relay-auth", () => {
  ...
  beforeEach(() => {
    prevGatewayToken = process.env.genesis_GATEWAY_TOKEN;
  ...
  it("accepts authenticated genesis relay probe responses", async () => {
    ...
    await withRelayServer(
      ...
      async ({ port }) => {
        ...
        const ok = await probeAuthenticatedgenesisRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-genesis-relay-token",
