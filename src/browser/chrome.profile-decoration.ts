...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".genesis-profile-decorated");
}
 ...
 */
export function decorategenesisProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_genesis_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_genesis_BROWSER_COLOR).toUpperCase();
