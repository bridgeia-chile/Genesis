...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["genesis_GATEWAY_TOKEN"]);
    process.env.genesis_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-genesis-relay-token");
    expect(headers["x-genesis-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-genesis-relay-token"];
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-genesis-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "genesis/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an genesis relay", async () => {
    const port = await getFreePort();
