    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "genesis", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-genesis-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-genesis-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "genesis", messages: [{ role: "user", content: "hi" }] },
          {
            "x-genesis-agent-id": "beta",
            "x-genesis-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from genesis.");
      }
