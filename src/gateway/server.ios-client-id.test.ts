...
beforeAll(async () => {
  previousToken = process.env.genesis_GATEWAY_TOKEN;
  process.env.genesis_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.genesis_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "genesis-ios", platform: "ios" },
  { clientId: "genesis-android", platform: "android" },
