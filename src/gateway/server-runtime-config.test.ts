...
describe("resolveGatewayRuntimeConfig", () => {
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.genesis_GATEWAY_TOKEN;
      delete process.env.genesis_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.genesis_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.genesis_GATEWAY_TOKEN;
      }
    });
    ...
    it.each([
      {
        ...
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or genesis_GATEWAY_TOKEN)",
      },
      {
