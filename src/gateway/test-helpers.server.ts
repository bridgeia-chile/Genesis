...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "genesis_SKIP_PROVIDERS",
  "genesis_SKIP_CRON",
  "genesis_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.genesis_STATE_DIR = path.join(tempHome, ".genesis");
  delete process.env.genesis_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.genesis_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.genesis_SKIP_GMAIL_WATCHER = "1";
  process.env.genesis_SKIP_CANVAS_HOST = "1";
  process.env.genesis_SKIP_CHANNELS = "1";
  process.env.genesis_SKIP_PROVIDERS = "1";
  process.env.genesis_SKIP_CRON = "1";
  process.env.genesis_TEST_MINIMAL_GATEWAY = "1";
  process.env.genesis_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "genesis-test-no-bundled-extensions")
    : "genesis-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "genesis-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".genesis-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__genesisTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__genesisTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["genesis_GATEWAY_TOKEN"]);
  const prev = process.env.genesis_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.genesis_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.genesis_GATEWAY_PASSWORD;
