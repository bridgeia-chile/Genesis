...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  const canvasHostEnabled =
    process.env.genesis_SKIP_CANVAS_HOST !== "1" && params.cfg.canvasHost?.enabled !== false;
  ...
  if (tailscaleMode === "funnel" && authMode !== "password") {
    throw new Error(
      "tailscale funnel requires gateway auth mode=password (set gateway.auth.password or genesis_GATEWAY_PASSWORD)",
    );
  }
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set genesis_GATEWAY_TOKEN/genesis_GATEWAY_PASSWORD)`,
    );
  }
