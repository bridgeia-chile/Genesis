...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: genesisConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: genesisConfig) {
  ...
  async function expectNoTokenGeneration(cfg: genesisConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          genesis_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
