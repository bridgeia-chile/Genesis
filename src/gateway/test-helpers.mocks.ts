...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `genesis-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.genesis_CONFIG_PATH = path.join(root, "genesis.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "genesis.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "genesis-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadgenesisPlugins: () => pluginRegistryState.registry,
  };
});
