...
const { creategenesisTools } = await import("../agents/genesis-tools.js");
...
type SessionSendTool = ReturnType<typeof creategenesisTools>[number];

function getSessionsSendTool(): SessionSendTool {
  const tool = creategenesisTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["genesis_GATEWAY_PORT", "genesis_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.genesis_GATEWAY_PORT = String(gatewayPort);
  process.env.genesis_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("genesis_CONFIG_PATH missing in gateway test environment");
    }
