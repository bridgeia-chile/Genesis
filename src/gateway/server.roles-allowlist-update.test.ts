...
const connectNodeClient = async (params: {
  ...
  const token = process.env.genesis_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("genesis_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".genesis", "restart-sentinel.json");
