...
function resolveGatewayTokenOrEnv(): string {
  const token =
    typeof (testState.gatewayAuth as { token?: unknown } | undefined)?.token === "string"
      ...
      : process.env.genesis_GATEWAY_TOKEN;
...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.genesis_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.genesis_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.genesis_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.genesis_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers service version fallback in hello-ok payload", async () => {
      await withRuntimeVersionEnv(
        {
          genesis_VERSION: " ",
          genesis_SERVICE_VERSION: "2.4.6-service",
    ...
    test("connect (req) handshake prefers genesis_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
          genesis_VERSION: "9.9.9-cli",
          genesis_SERVICE_VERSION: "2.4.6-service",
          npm_package_version: "1.0.0-package",
        },
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `genesis-test-device-${randomUUID()}.json`),
        nonce,
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.genesis_GATEWAY_TOKEN;
      process.env.genesis_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
    ...
    try {
      await withGatewayServer(async ({ port }) => {
        ...
        const { device } = await createSignedDevice({
          ...
          identityPath: path.join(os.tmpdir(), `genesis-controlui-device-${randomUUID()}.json`),
          nonce: String(nonce),
        });
        const res = await connectReq(ws, {
          token: "secret",
  ...
  test("skips pairing for operator scope upgrades when shared token auth is valid", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "genesis-device-scope-"));
  ...
  test("auto-approves loopback scope upgrades for control ui clients", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "genesis-device-token-scope-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
  ...
  test("allows operator shared auth with legacy paired metadata", async () => {
    ...
    const { approveDevicePairing, getPairedDevice, listDevicePairing, requestDevicePairing } =
      await import("../infra/device-pairing.js");
    const identityDir = await mkdtemp(join(tmpdir(), "genesis-device-legacy-meta-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
  ...
  test("allows shared-auth scope escalation even when paired metadata is legacy-shaped", async () => {
    ...
    const identityDir = await mkdtemp(join(tmpdir(), "genesis-device-legacy-"));
    const identity = loadOrCreateDeviceIdentity(join(identityDir, "device.json"));
    const devicePublicKey = publicKeyRawBase64UrlFromPem(identity.publicKeyPem);
    const seeded = await requestDevicePairing({
      deviceId: identity.deviceId,
