...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.genesis_SKIP_CHANNELS;
    prevSkipGmail = process.env.genesis_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.genesis_SKIP_PROVIDERS;
    process.env.genesis_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.genesis_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.genesis_SKIP_GMAIL_WATCHER;
    } else {
      process.env.genesis_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.genesis_SKIP_PROVIDERS;
    } else {
      process.env.genesis_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
