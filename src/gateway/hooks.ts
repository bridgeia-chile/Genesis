...
export function resolveHooksConfig(cfg: genesisConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: genesisConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-genesis-token"] === "string"
      ? req.headers["x-genesis-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
