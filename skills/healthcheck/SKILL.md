---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for genesis deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, genesis cron scheduling for periodic checks, or version status checks on a machine running genesis (laptop, workstation, Pi, VPS).
---

# genesis Host Hardening

## Overview

Assess and harden the host running genesis, then align it to a user-defined risk tolerance without breaking access. Use genesis security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim genesis changes the host firewall, SSH, or OS updates; it does not.
...
5. genesis gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if genesis can access sensitive data.
...
### 2) Run genesis security audits (read-only)

As part of the default read-only checks, run `genesis security audit --deep`. Only offer alternatives if the user requests them:

1. `genesis security audit` (faster, non-probing)
2. `genesis security audit --json` (structured output)

Offer to apply genesis safe defaults (numbered):

1. `genesis security audit --fix`

Be explicit that `--fix` only tightens genesis defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check genesis version/update status (read-only)

As part of the default read-only checks, run `genesis update status`.
...
- Credential hygiene notes (location of genesis creds, prefer disk encryption)
...
After genesis install or first hardening pass, run at least one baseline audit and version check:

- `genesis security audit`
- `genesis security audit --deep`
- `genesis update status`

Ongoing monitoring is recommended. Use the genesis cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `genesis cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `genesis update status`
...
Before creating, `genesis cron list` and match on exact `name`. If found, `genesis cron edit <id> ...`.
If not found, `genesis cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `genesis update status` (preferred for source checkouts and channels)
2. `npm view genesis version` (published npm version)

## genesis command accuracy

Use only supported commands and flags:

- `genesis security audit [--deep] [--fix] [--json]`
- `genesis status` / `genesis status --deep`
- `genesis health --json`
- `genesis update status`
- `genesis cron add|list|runs|run`

Do not invent CLI flags or imply genesis enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by genesis compaction:
