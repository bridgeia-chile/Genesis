...
# genesis Auth Widget for Termux
...
SERVER="${genesis_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking genesis auth..."

STATUS=$(ssh "$SERVER" '$HOME/genesis/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/genesis/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|genesis_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/genesis/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|genesis_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: genesis won't work until re-auth"
                ;;
        esac
        ;;
