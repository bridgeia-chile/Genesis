...
BASE_IMAGE="${BASE_IMAGE:-genesis-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-genesis-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=genesis.sandbox=1)
NOTE
