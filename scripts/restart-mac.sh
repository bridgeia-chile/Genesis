#!/usr/bin/env bash
# Reset genesis like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${genesis_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="genesis.app/Contents/MacOS/genesis"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/genesis"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/genesis"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/genesis"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.genesis.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/genesis-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${genesis_RESTART_LOG:-/tmp/genesis-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${genesis_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.genesis/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  genesis_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node genesis.mjs daemon install --force --runtime node"
      log "  node genesis.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.genesis/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.genesis.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing genesis instances"
kill_all_genesis
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product genesis"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/genesis.app" ]]; then
    APP_BUNDLE="/Applications/genesis.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/genesis.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/genesis.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/genesis.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set genesis_APP_BUNDLE to your installed genesis.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node genesis.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node genesis.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".genesis", "genesis.json"), "utf8");
