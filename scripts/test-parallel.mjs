...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/genesis-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// genesis_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.genesis_TEST_VM_FORKS === "1" ||
  (process.env.genesis_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks);
const disableIsolation = process.env.genesis_TEST_NO_ISOLATE === "1";
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.genesis_TEST_SHARDS ?? "", 10);
...
const silentArgs =
  process.env.genesis_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.genesis_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
  ...
  rawTestProfile === "serial"
    ? rawTestProfile
    : "normal";
const overrideWorkers = Number.parseInt(process.env.genesis_TEST_WORKERS ?? "", 10);
const resolvedOverride =
  Number.isFinite(overrideWorkers) && overrideWorkers > 0 ? overrideWorkers : null;
// Keep gateway serial on Windows CI and CI by default; run in parallel locally
// for lower wall-clock time. CI can opt in via genesis_TEST_PARALLEL_GATEWAY=1.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.genesis_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
  (isCI && process.env.genesis_TEST_PARALLEL_GATEWAY !== "1");
const parallelRuns = keepGatewaySerial ? runs.filter((entry) => entry.name !== "gateway") : runs;
const serialRuns = keepGatewaySerial ? runs.filter((entry) => entry.name === "gateway") : [];
const hostCpuCount = os.cpus().length;
const baseLocalWorkers = Math.max(4, Math.min(16, hostCpuCount));
const loadAwareDisabledRaw = process.env.genesis_TEST_LOAD_AWARE?.trim().toLowerCase();
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.genesis_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
...
function resolveReportDir() {
  const raw = process.env.genesis_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
