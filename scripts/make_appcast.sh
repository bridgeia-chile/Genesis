...
ZIP=${1:?"Usage: $0 genesis-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/genesis/genesis/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^genesis-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/genesis/genesis/releases/download/v${VERSION}/"}
