...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/genesis/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "genesis Re-Auth" -c "After getting key, run: ssh $SERVER '~/genesis/scripts/mobile-reauth.sh'" --id genesis-auth
        ;;
    *)
