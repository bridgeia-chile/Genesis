...
IMAGE_NAME="genesis-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/genesis-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$genesis_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/genesis-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/genesis-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$genesis_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/genesis-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.genesis/workspace"
    config_path="$HOME/.genesis/genesis.json"
    sessions_dir="$HOME/.genesis/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$genesis_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.genesis/genesis.json" <<'"'"'JSON'"'"'
{
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$genesis_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$genesis_ENTRY\" configure --section skills" send_skills_flow
