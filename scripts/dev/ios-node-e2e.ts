...
const urlRaw = getArg("--url") ?? process.env.genesis_GATEWAY_URL;
const token = getArg("--token") ?? process.env.genesis_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.genesis_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  ...
  console.error(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: genesis_GATEWAY_URL / genesis_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      ...
      platform: "dev",
      mode: "cli",
      instanceId: "genesis-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "genesis E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
