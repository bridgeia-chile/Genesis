#!/bin/bash
# Setup genesis Auth Management System
...
echo "=== genesis Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'genesis-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  genesis message: Send warning via genesis itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/genesis-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/genesis-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/genesis-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now genesis-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status genesis-auth-monitor.timer"
echo ""
echo "Quick commands:"
