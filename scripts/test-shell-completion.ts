 ...
 * `genesis update`. Use it to verify the completion installation flow
...
\`genesis update\`, \`genesis doctor\`, and \`genesis onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# genesis Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`genesis completion --install\` later to enable.`));
    return;
  }
