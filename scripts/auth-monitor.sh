...
#
# Suggested cron: */30 * * * * /home/admin/genesis/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send genesis notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., genesis-alerts)
...
STATE_FILE="$HOME/.genesis/auth-monitor-state"
...
send_notification() {
    ...
    # Send via genesis if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use genesis
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via genesis to $NOTIFY_PHONE..."
            genesis send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! genesis is down. Run: ssh l36 '~/genesis/scripts/mobile-reauth.sh'" "urgent"
    exit 1
