...
function runTests() {
  const isolatedLock =
    process.env.genesis_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `genesis-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      genesis_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.genesis_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
