...
SMOKE_PREVIOUS_VERSION="${genesis_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${genesis_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (genesis_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${genesis_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${genesis_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
