...
This document describes how the genesis ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`genesis acp` exposes an ACP agent over stdio and forwards prompts to a running
genesis Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `genesis acp` and speaks ACP messages over stdio.
...
`genesis acp` resolves the Gateway URL and auth from CLI flags or config:
