...
read_when:
  - You need detailed behavior for genesis onboard
...
This page is the full reference for `genesis onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.bridgeia-chile/Genesis.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `genesis doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `genesis pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `genesis health`.
    - `genesis status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.genesis/.env` so launchd can read it.
...
- OAuth credentials: `~/.genesis/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.genesis/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.genesis/credentials/oauth.json` (or `$genesis_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`genesis onboard`](/cli/onboard)
