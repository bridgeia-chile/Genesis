...
- **Tailoring lives outside the repo:** `~/.genesis/workspace` (workspace) + `~/.bridgeia-chile/Genesis.json` (config).
...
- **Config:** `~/.bridgeia-chile/Genesis.json` (JSON/JSON5-ish)
...
If you don’t have a global install yet, run it via `pnpm genesis setup`.
