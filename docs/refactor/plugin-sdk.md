...
- Publish as `genesis/plugin-sdk` (or export from core under `genesis/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `genesisPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: genesisConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: genesisConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: genesisConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: genesisConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: genesisConfig): string;
  };
};
```
...
- Introduce `genesis/plugin-sdk`.
- Add `api.runtime` to `genesisPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `genesisRuntime: ">=2026.2.0"`).
