...
genesis can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere genesis can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, genesis defaults
...
- If `provider` is **unset**, genesis prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, genesis retries with MP3.
