---
title: "Memory"
summary: "How genesis memory works (workspace files + automatic memory flush)"
read_when:
...
genesis memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.genesis/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

genesis exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, genesis triggers a **silent,
...
genesis can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, genesis auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. genesis
...
BM25 + vectors + reranking. Markdown stays the source of truth; genesis shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.genesis/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.genesis/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, genesis retries with `qmd query`. If QMD fails or the binary is
  missing, genesis automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- genesis does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - genesis sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index genesis
    uses), run a one-off query with the agent’s XDG dirs.

    genesis’s QMD state lives under your **state dir** (defaults to `~/.genesis`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    genesis uses:

    ```bash
    # Pick the same state dir genesis uses
...
- When `scope` denies a search, genesis logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, genesis exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.genesis/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.genesis/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, genesis automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, genesis combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, genesis falls back to vector-only search.
...
genesis can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.genesis/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, genesis stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, genesis logs the
