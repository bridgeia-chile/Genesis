...
summary: "Architecture of genesis's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in genesis
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how genesis integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.
...
| `pi-tui`          | Terminal UI components (used in genesis's local TUI mode)                                             |
...
2. **Custom Replacements**: genesis replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **genesis Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `genesis_LIVE_TEST=1`)
