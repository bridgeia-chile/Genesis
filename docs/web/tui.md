...
```bash
genesis tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `genesis tui --deliver`
...
- Check the Gateway logs: `genesis logs --follow`.
- Confirm the agent can run: `genesis status` and `genesis models status`.
...
- No agents in picker: check `genesis agents list` and your routing config.
