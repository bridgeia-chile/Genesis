---
summary: "CLI reference for `genesis security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `genesis.browserConfigEpoch`) and recommends `genesis sandbox recreate --browser --all`.
...
```bash
genesis security audit --json | jq '.summary'
genesis security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
genesis security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
