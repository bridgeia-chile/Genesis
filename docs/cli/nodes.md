---
summary: "CLI reference for `genesis nodes` (list/status/approve/invoke, camera/canvas/screen)"
read_when:
...
genesis nodes run --node <id|name|ip> <command...>
genesis nodes run --raw "git status"
genesis nodes run --agent main --node <id|name|ip> --raw "git status"
```
