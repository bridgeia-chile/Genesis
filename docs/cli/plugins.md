---
summary: "CLI reference for `genesis plugins` (list, install, uninstall, enable/disable, doctor)"
read_when:
...
Bundled plugins ship with genesis but start disabled. Use `plugins enable` to
activate them.

All plugins must ship a `genesis.plugin.json` file with an inline JSON Schema
...
genesis plugins install <npm-spec> --pin
```
...
state dir extensions root (`$genesis_STATE_DIR/extensions/<id>`). Use
...
genesis plugins update <id>
genesis plugins update --all
genesis plugins update <id> --dry-run
```
