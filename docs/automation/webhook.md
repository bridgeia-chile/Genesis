...
read_when:
  ...
  - Wiring external systems into genesis
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your genesis config directory (typically `~/.genesis/hooks/transforms`).
...
- `genesis webhooks gmail setup` writes `hooks.gmail` config for `genesis webhooks gmail run`.
