...
- Jobs persist under `~/.genesis/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.genesis/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `genesis cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, genesis applies a
...
If `delivery` is omitted for isolated jobs, genesis defaults to `announce`.
...
- Job store: `~/.genesis/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.genesis/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
genesis cron edit <jobId> --agent ops
...
```bash
genesis system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`genesis system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `genesis_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- genesis applies exponential retry backoff for recurring jobs after consecutive errors:
