...
genesis system heartbeat last
genesis logs --follow
genesis config get agents.defaults.heartbeat
genesis channels status --probe
```
...
```bash
genesis config get agents.defaults.heartbeat.activeHours
genesis config get agents.defaults.heartbeat.activeHours.timezone
genesis config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
genesis cron list
genesis logs --follow
```
