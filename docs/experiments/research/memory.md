...
read_when:
  - Designing workspace memory (~/.genesis/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep genesis integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.genesis/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.genesis/workspace` as canonical human-readable memory.
...
```
~/.genesis/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in genesis**, but keep a separable core library.

### Why integrate into genesis?

- genesis already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `genesis memory recall "…" --k 25 --since 30d`
  - `genesis memory reflect --since 7d`
...
Pragmatic take for `~/.genesis/workspace`:
