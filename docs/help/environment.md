...
4. **Config `env` block** in `~/.bridgeia-chile/Genesis.json` (applied only if missing).
...
| `genesis_CONFIG_PATH` | Override the config file path (default `~/.bridgeia-chile/Genesis.json`).                                                                                                             |
