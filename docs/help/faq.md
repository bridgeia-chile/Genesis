---
summary: "Frequently asked questions about genesis setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up genesis?](#whats-the-recommended-way-to-install-and-set-up-genesis)
  ...
  - [I can't access docs.genesis.ai (SSL error). What now?](#i-cant-access-docsgenesisai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or genesis not recognized](#windows-install-says-git-not-found-or-genesis-not-recognized)
  - [The docs didn't answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install genesis on Linux?](#how-do-i-install-genesis-on-linux)
  - [How do I install genesis on a VPS?](#how-do-i-install-genesis-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask genesis to update itself?](#can-i-ask-genesis-to-update-itself)
  ...
  - [If I buy a Mac mini to run genesis, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-genesis-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different genesis instances?](#can-multiple-people-use-one-whatsapp-number-with-different-genesis-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run genesis on a dedicated machine?](#how-important-is-it-to-run-genesis-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run genesis in a VM and what are the requirements](#can-i-run-genesis-in-a-vm-and-what-are-the-requirements)
- [What is genesis?](#what-is-genesis)
  - [What is genesis, in one paragraph?](#what-is-genesis-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for genesis](#what-are-the-top-five-everyday-use-cases-for-genesis)
  - [Can genesis help with lead gen outreach ads and blogs for a SaaS](#can-genesis-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can genesis run tasks on a schedule or continuously in the background?](#can-genesis-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with genesis saved locally?](#is-all-data-used-with-genesis-saved-locally)
  - [Where does genesis store its data?](#where-does-genesis-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall genesis?](#how-do-i-completely-uninstall-genesis)
...
- [Config basics](#config-basics)
  ...
  - [Can the genesis browser run headless?](#can-the-genesis-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two genesis instances talk to each other (local + VPS)?](#can-two-genesis-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does genesis load environment variables?](#how-does-genesis-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of genesis instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-genesis-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset genesis but keep it installed?](#how-do-i-completely-reset-genesis-but-keep-it-installed)
  ...
  - [Why doesn't genesis reply in a group?](#why-doesnt-genesis-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do genesis, Flawd, and Krill use for models?](#what-do-genesis-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `genesis gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-genesis-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `genesis gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-genesis-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run genesis in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-genesis-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart genesis?](#i-closed-my-terminal-on-windows-how-do-i-restart-genesis)
  ...
  - [ELI5: `genesis gateway restart` vs `genesis gateway`](#eli5-genesis-gateway-restart-vs-genesis-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose genesis to inbound DMs?](#is-it-safe-to-expose-genesis-to-inbound-dms)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/bridgeia-chile/Genesis-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   genesis health --json
   genesis health --verbose   # shows the target URL + config path on errors
   ```
...
[https://github.com/bridgeia-chile/Genesis/issues](https://github.com/bridgeia-chile/Genesis/issues)
[https://github.com/bridgeia-chile/Genesis/pulls](https://github.com/bridgeia-chile/Genesis/pulls)
...
- `genesis status`: quick snapshot of gateway/agent health + basic config.
- `genesis models status`: checks provider auth + model availability.
- `genesis doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `genesis status --all`, `genesis logs --follow`,
`genesis gateway status`, `genesis health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up genesis
...
```bash
git clone https://github.com/bridgeia-chile/Genesis.git
cd genesis
pnpm install
pnpm build
...
If you don't have a global install yet, run it via `pnpm genesis onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `genesis_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `genesis config get gateway.auth.token` (or generate one: `genesis doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `genesis gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `genesis gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install genesis on the new machine.
2. Copy `$genesis_STATE_DIR` (default: `~/.genesis`) from the old machine.
3. Copy your workspace (default: `~/.genesis/workspace`).
4. Run `genesis doctor` and restart the Gateway service.
...
under `~/.genesis/` (for example `~/.genesis/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-genesis-store-its-data),
...
[https://github.com/bridgeia-chile/Genesis/blob/main/CHANGELOG.md](https://github.com/bridgeia-chile/Genesis/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I can't access docs.genesis.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.genesis.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.genesis.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsgenesisai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/bridgeia-chile/Genesis/tree/main/docs](https://github.com/bridgeia-chile/Genesis/tree/main/docs)
...
### Windows install says git not found or genesis not recognized
...
**2) genesis is not recognized after install**
...
### How do I install genesis on Linux
...
### How do I install genesis on a VPS
...
### Can I ask genesis to update itself
...
```bash
genesis update
genesis update status
genesis update --channel stable|beta|dev
genesis update --tag <dist-tag|version>
genesis update --no-restart
```

If you must automate from an agent:

```bash
genesis update --yes --no-restart
genesis gateway restart
```
...
`genesis onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run genesis with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `genesis models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `genesis models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `genesis models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. genesis no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so genesis can keep replying while a provider is rate-limited.
...
genesis supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. genesis fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `genesis.json`.

Steps:

1. Enable the plugin: `genesis plugins enable google-gemini-cli-auth`
2. Login: `genesis models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. genesis needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. genesis runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run genesis can I connect it to my MacBook Pro
...
- Use `genesis nodes status` / `genesis nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but genesis authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `genesis logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different genesis instances
...
If you run genesis via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the genesis code install. Your state
(`~/.genesis`) and workspace (`~/.genesis/workspace`) stay untouched.
...
**genesis-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run genesis on a dedicated machine
...
genesis is lightweight. For a basic Gateway + one chat channel:
...
### Can I run genesis in a VM and what are the requirements
...
### What is genesis in one paragraph

genesis is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

genesis is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for genesis
...
### Can genesis help with lead gen outreach ads and blogs for a SaaS
...
genesis draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

genesis is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use genesis when you
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.genesis/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.bridgeia-chile/Genesis.json`). Precedence is `<workspace>/skills` > `~/.genesis/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.bridgeia-chile/Genesis.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.genesis/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which genesis treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `genesis_SKIP_CRON` is not set.
...
### Can genesis run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.genesis.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-genesis-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. genesis can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.genesis/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "genesis": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured genesis workspace); genesis treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.genesis/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `genesis_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `genesis_DOCKER_APT_PACKAGES`.
...
genesis memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

genesis also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, genesis auto-selects a provider when it
...
configured and present, genesis
prefers `local`.
...
### Is all data used with genesis saved locally

No - **genesis's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.genesis` + your workspace directory).
...
### Where does genesis store its data

Everything lives under `$genesis_STATE_DIR` (default: `~/.genesis`):

| Path                                                            | Purpose                                                      |
| --------------------------------------------------------------- | ------------------------------------------------------------ |
| `$genesis_STATE_DIR/genesis.json`                             | Main config (JSON5)                                          |
| `$genesis_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use) |
| `$genesis_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys)                             |
| `$genesis_STATE_DIR/agents/<agentId>/agent/auth.json`          | Runtime auth cache (managed automatically)                   |
| `$genesis_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)      |
| `$genesis_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                        |
| `$genesis_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                     |
| `$genesis_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                 |

Legacy single-agent path: `~/.genesis/agent/*` (migrated by `genesis doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.genesis/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.genesis`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.genesis`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.genesis/skills`).

Default workspace is `~/.genesis/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.genesis/workspace" } },
}
```
...
Do **not** commit anything under `~/.genesis` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall genesis
...
`workspace` to the repo root. The genesis repo is just source code; keep the
...
genesis reads an optional **JSON5** config from `$genesis_CONFIG_PATH` (default: `~/.bridgeia-chile/Genesis.json`):

```
$genesis_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.genesis/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `genesis_GATEWAY_TOKEN`).
...
genesis enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `genesis doctor --generate-gateway-token`.
...
key. **Recommended:** run `genesis configure --section web` to store it in
...
- Daemons read env vars from `~/.genesis/.env` (or the service environment).
...
### Can the genesis browser run headless
...
- Gateway is running: `genesis gateway status`
- Gateway health: `genesis status`
- Channel health: `genesis channels status`
...
### Can two genesis instances talk to each other local VPS
...
`genesis agent --message ... --deliver`, targeting a chat where the other bot
...
```bash
genesis agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
- Restore from backup (git or a copied `~/.bridgeia-chile/Genesis.json`).
- If you have no backup, re-run `genesis doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `genesis config set` for small changes.
- Use `genesis configure` for interactive edits.
...
### How does genesis load environment variables

genesis reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.genesis/.env` (aka `$genesis_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.genesis/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`genesis_LOAD_SHELL_ENV=1`, `genesis_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`genesis models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means genesis won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.genesis/.env`:
...
### Is there a way to make a team of genesis instances one CEO and many agents
...
### How do I completely reset genesis but keep it installed
...
- If you used profiles (`--profile` / `genesis_PROFILE`), reset each state dir (defaults are `~/.genesis-<profile>`).
- Dev reset: `genesis gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
No. genesis runs on **your own account**, so if you're in the group, genesis can see it.
...
```bash
genesis directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesn't genesis reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.genesis/agents/<agentId>/sessions/`.
...
- Use `genesis doctor` to spot stray workspaces and profile mismatches.
...
genesis's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, genesis currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `genesis models set ...` (updates just model config)
- `genesis configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.bridgeia-chile/Genesis.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `genesis doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do genesis, Flawd, and Krill use for models

- **genesis + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
Yes. genesis ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run `genesis agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so genesis can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.genesis/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.genesis/agent/*` (migrated by `genesis doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.genesis/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `genesis models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `genesis models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `genesis models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.genesis/.env` on the **gateway host**.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), genesis will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: genesis now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
genesis uses provider-prefixed IDs like:
...
genesis may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `genesis models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
genesis models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
genesis models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key what's the difference

genesis supports both:
...
```
--port > genesis_GATEWAY_PORT > gateway.port > default 18789
```

### Why does genesis gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `genesis gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does genesis gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `genesis_STATE_DIR` mismatch).
...
genesis enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `genesis gateway --port <port>`.

### How do I run genesis in remote mode client connects to a Gateway elsewhere
...
- `genesis gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `genesis.control.settings.v1`.

Fix:

- Fastest: `genesis dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `genesis doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `genesis_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- Still stuck? Run `genesis status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `genesis_CONFIG_PATH` (per-instance config)
- `genesis_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `genesis --profile <name> …` per instance (auto-creates `~/.genesis-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `genesis --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.genesis.*`, `genesis-gateway-<profile>.service`, `genesis Gateway (<profile>)`).
...
```
genesis tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/bridgeia-chile/Genesis-YYYY-MM-DD.log
```
...
- macOS: `$genesis_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.genesis/logs/...`; profiles use `~/.genesis-<profile>/logs/...`)
- Linux: `journalctl --user -u genesis-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "genesis Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `genesis gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart genesis
...
1. Is the Gateway running? `genesis gateway status`
2. Is the Gateway healthy? `genesis status`
3. Does the UI have the right token? `genesis dashboard`
...
```bash
genesis channels status
genesis channels logs --channel telegram
```
...
### ELI5 genesis gateway restart vs genesis gateway

- `genesis gateway restart`: restarts the **background service** (launchd/systemd).
- `genesis gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `genesis gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [genesis assistant setup](/start/genesis) and [Agent send](/tools/agent-send).

CLI sending:

```bash
genesis message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose genesis to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `genesis pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `genesis pairing list <channel>` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `genesis doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. genesis only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
genesis pairing approve whatsapp <code>
```

List pending requests:

```bash
genesis pairing list whatsapp
```
...
genesis blocks **cross-provider** messaging by default. If a tool call is bound
