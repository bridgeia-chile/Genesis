...
Deploy genesis on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/genesis` — Control UI
...
- `genesis_STATE_DIR=/data/.genesis` (recommended)
- `genesis_WORKSPACE_DIR=/data/workspace` (recommended)
- `genesis_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your genesis state + workspace so you can migrate to another host without losing config or memory.
