...
Deploy genesis on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/genesis/genesis)
...
repository configures everything needed to run genesis:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: genesis_STATE_DIR
        value: /data/.genesis
      - key: genesis_WORKSPACE_DIR
        value: /data/workspace
      - key: genesis_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/genesis`.
...
If you use the original genesis repository, Render will not auto-deploy your genesis. To update it, run a manual Blueprint sync from the dashboard.
...
For genesis, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any genesis host.
