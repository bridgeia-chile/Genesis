---
title: IRC
description: Connect genesis to IRC channels and direct messages.
summary: "IRC plugin setup, access controls, and troubleshooting"
read_when:
  - You want to connect genesis to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want genesis in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.bridgeia-chile/Genesis.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, genesis defaults to **mention-gating** in group contexts.
