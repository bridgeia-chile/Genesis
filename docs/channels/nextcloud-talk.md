...
```bash
genesis plugins install @genesis/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "genesis" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `genesis pairing list nextcloud-talk`
  - `genesis pairing approve nextcloud-talk <CODE>`
