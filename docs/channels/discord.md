...
You will need to create a new application with a bot, add the bot to your server, and pair it to genesis. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "genesis".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your genesis agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to genesis in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with genesis. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Step 0: Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running genesis before messaging your agent.

```bash
genesis config set channels.discord.token '"YOUR_BOT_TOKEN"' --json
genesis config set channels.discord.enabled true --json
genesis gateway
```

    If genesis is already running as a background service, use `genesis gateway restart` instead.

  </Step>

  <Step title="Configure genesis and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your genesis agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```bash
genesis pairing list discord
genesis pairing approve discord <CODE>
```
...
Discord forum and media channels only accept thread posts. genesis supports two ways to create them:
...
- Use `genesis message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
genesis message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
genesis message thread create --channel discord --target channel:<forumId> \
...
genesis supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- genesis adds a trigger button automatically
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `genesis security audit` warns when name/tag entries are used
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in genesis config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces genesis auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    genesis can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, genesis skips the preview stream to avoid double streaming.
...
```
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while genesis is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/genesis",
    },
  },
}
```
...
genesis uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires Carbon component instances), while legacy `embeds` remain available but are not recommended.
...
genesis can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. genesis generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; genesis converts to OGG/Opus when needed.
...
- If command deploy/state is stale, restart gateway and re-check with `genesis channels status --probe`.
