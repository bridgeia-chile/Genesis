...
read_when:
  - You want genesis to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables genesis to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`genesis onboard`) and `genesis channels add` list optional channel plugins.
...
```bash
genesis plugins install @genesis/nostr
```

Use a local checkout (dev workflows):

```bash
genesis plugins install --link <path-to-genesis>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "genesis@example.com",
        "lud16": "genesis@example.com"
      }
    }
  }
}
