...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects genesis to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
genesis plugins install @genesis/feishu
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/feishu
```
...
If you just installed genesis, run the wizard:
...
2. Configuring app credentials in genesis
...
1. You already ran `genesis channels add` for Feishu
2. The gateway is running (`genesis gateway status`)
...
Edit `~/.bridgeia-chile/Genesis.json`:
...
```bash
genesis pairing approve feishu <CODE>
```
...
2. Run `genesis logs --follow` and look for `chat_id`
...
2. Run `genesis logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `genesis gateway status`  | Show gateway status           |
| `genesis gateway install` | Install/start gateway service |
| `genesis gateway stop`    | Stop gateway service          |
| `genesis gateway restart` | Restart gateway service       |
| `genesis logs --follow`   | Tail gateway logs             |
...
4. Check logs: `genesis logs --follow`
...
5. Ensure the gateway is running: `genesis gateway status`
6. Check logs: `genesis logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.genesis/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.genesis/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
