...
Tlon is a decentralized messenger built on Urbit. genesis connects to your Urbit ship and can
...
```bash
genesis plugins install @genesis/tlon
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/tlon
```
...
By default, genesis blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `genesis message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, genesis replies in-thread.
