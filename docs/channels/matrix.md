...
Matrix is an open, decentralized messaging protocol. genesis connects as a Matrix **user**
...
```bash
genesis plugins install @genesis/matrix
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `genesis plugins install @genesis/matrix`
   - From a local checkout: `genesis plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: genesis calls the same
     login endpoint, stores the access token in `~/.genesis/credentials/matrix/credentials.json`,
...
- On first connection, genesis requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  genesis logs a warning.
...
`~/.genesis/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `genesis pairing list matrix`
  - `genesis pairing approve matrix <CODE>`
...
- On startup, genesis resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
