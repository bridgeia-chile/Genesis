---
summary: "Mattermost bot setup and genesis config"
read_when:
...
```bash
genesis plugins install @genesis/mattermost
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/mattermost
```
...
4. Configure genesis and start the gateway.
...
- Approve via:
  - `genesis pairing list mattermost`
  - `genesis pairing approve mattermost <CODE>`
...
Use these target formats with `genesis message send` or cron/webhooks:
