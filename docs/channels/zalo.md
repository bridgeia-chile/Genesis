...
- Install via CLI: `genesis plugins install @genesis/zalo`
...
1. Install the Zalo plugin:
   - From a source checkout: `genesis plugins install ./extensions/zalo`
   - From npm (if published): `genesis plugins install @genesis/zalo`
...
- Approve via:
  - `genesis pairing list zalo`
  - `genesis pairing approve zalo <CODE>`
...
- Example: `genesis message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `genesis channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `genesis logs --follow`
