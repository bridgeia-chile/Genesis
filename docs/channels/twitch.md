...
read_when:
  - Setting up Twitch chat integration for genesis
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. genesis connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
genesis plugins install @genesis/twitch
```

Local checkout (when running from a git repo):

```bash
genesis plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `genesis_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "genesis", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use genesis_TWITCH_ACCESS_TOKEN env var)
...
```bash
genesis_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
