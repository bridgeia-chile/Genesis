...
## Quick check: `genesis security audit`
...
genesis security audit --deep
genesis security audit --fix
genesis security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

genesis is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
genesis assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.genesis`, including `genesis.json`), treat them as a trusted operator.
...
If you run `--deep`, genesis also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.genesis/credentials/<channel>-allowFrom.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full genesis state                               | filesystem perms on `~/.genesis`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.bridgeia-chile/Genesis.json`                                                   | yes      |
...
`genesis security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`genesis security audit` includes `config.insecure_or_dangerous_flags` when any
...
gateway:
  ...
  auth:
    mode: password
    password: ${genesis_GATEWAY_PASSWORD}
```
...
genesis stores session transcripts on disk under `~/.genesis/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.genesis` (see the audit section below). If you need
...
genesis can refresh the skills list mid-session:
...
- If you install plugins from npm (`genesis plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.genesis/extensions/<pluginId>/` (or `$genesis_STATE_DIR/extensions/<pluginId>/`).
  - genesis uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
genesis pairing approve <channel> <code>
```
...
By default, genesis routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
genesis has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.genesis/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.genesis or your logs.”

## Unsafe external content bypass flags

genesis includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.bridgeia-chile/Genesis.json`: `600` (user read/write only)
- `~/.genesis`: `700` (user only)

`genesis doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `genesis_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__genesis__/canvas/` and `/__genesis__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_genesis-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `genesis_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `genesis doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `genesis_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `genesis_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), genesis
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. genesis verifies the identity by resolving the
...
- genesis will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.genesis/` (or `$genesis_STATE_DIR/`) may contain secrets or private data:

- `genesis.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `genesis status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@genesis", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.genesis/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `genesis` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only genesis clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default genesis limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `genesis gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `genesis_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/bridgeia-chile/Genesis-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.genesis/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `genesis security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + genesis version
