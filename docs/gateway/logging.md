...
- Default rolling log file is under `/tmp/genesis/` (one file per day): `genesis-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.bridgeia-chile/Genesis.json`:
  - `logging.file`
  - `logging.level`
...
`genesis gateway` supports a per-gateway style switch:
