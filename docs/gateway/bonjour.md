...
genesis uses Bonjour (mDNS / DNS‑SD) as a **LAN‑only convenience** to discover
...
- Set `gateway.bind: "tailnet"` in `~/.bridgeia-chile/Genesis.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_genesis-gw._tcp`.
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `genesis` entrypoint)
...
- `genesis_DISABLE_BONJOUR=1` disables advertising (legacy: `genesis_DISABLE_BONJOUR`).
- `gateway.bind` in `~/.bridgeia-chile/Genesis.json` controls the Gateway bind mode.
- `genesis_SSH_PORT` overrides the SSH port advertised in TXT (legacy: `genesis_SSH_PORT`).
- `genesis_TAILNET_DNS` publishes a MagicDNS hint in TXT (legacy: `genesis_TAILNET_DNS`).
