...
description: "Complete field-by-field reference for ~/.bridgeia-chile/Genesis.json"
summary: "Complete reference for every genesis config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.bridgeia-chile/Genesis.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — genesis uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `genesis doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["genesis-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `genesis plugins install @genesis/mattermost`.
...
genesis spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@genesis"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `genesis.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, genesis auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, genesis assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "genesis-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.genesis/sandboxes`
...
noVNC observer access uses VNC auth by default and genesis emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `genesis-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.genesis/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@genesis"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:genesis`   | All built-in tools (excludes provider plugins)                                           |
...
genesis uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.genesis/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `genesis_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `genesis onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `genesis onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `genesis onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `genesis onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `genesis onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `genesis onboard --auth-choice minimax-api`.
...
- Loaded from `~/.genesis/extensions`, `<workspace>/.genesis/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or genesis_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `genesis_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.genesis-dev` + port `19001`), `--profile <name>` (uses `~/.genesis-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.genesis/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-genesis-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `genesis_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or genesis_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__genesis__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__genesis__/a2ui/`
...
- Hostname defaults to `genesis`. Override with `genesis_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.genesis/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `genesis dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.genesis/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${genesis_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.genesis/credentials/oauth.json`.
...
- Default log file: `/tmp/bridgeia-chile/Genesis-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `genesis doctor --fix` can strip unknown keys).
