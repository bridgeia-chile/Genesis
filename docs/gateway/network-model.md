...
Most operations flow through the Gateway (`genesis gateway`), a single long-running
...
- Loopback first: the Gateway WS defaults to `ws://127.0.0.1:18789`. The wizard generates a gateway token by default, even for loopback. For tailnet access, run `genesis gateway --bind tailnet --token ...` because tokens are required for non-loopback binds.
