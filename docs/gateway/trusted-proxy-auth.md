...
read_when:
  - Running genesis behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of genesis
...
- You run genesis behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. genesis checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. genesis extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://genesis.example.com
    to: http://genesis-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`genesis security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update genesis config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `genesis security audit` and review findings
