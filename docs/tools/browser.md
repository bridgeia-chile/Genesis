...
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `genesis` for the isolated managed browser.
...
## Profiles: `genesis` vs `chrome`

- `genesis`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the genesis
...
Browser settings live in `~/.bridgeia-chile/Genesis.json`.
...
- If you override the Gateway port (`gateway.port` or `genesis_GATEWAY_PORT`),
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "genesis"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `genesis` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
CDP endpoints over HTTPS. You can point a genesis browser profile at a
...
- If browser control is enabled and no auth is configured, genesis auto-generates `gateway.auth.token` on startup and persists it to config.
...
genesis can also drive **your existing Chrome tabs** (no separate “genesis” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **genesis Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
- “Load unpacked” → select the directory printed by `genesis browser extension path`
...
error. ARIA snapshots and basic screenshots still work for genesis-managed Chrome.
For the Chrome extension relay driver, ARIA snapshots and screenshots require Playwright.

If you see `Playwright is not available in this gateway build`, install the full
Playwright package (not `playwright-core`) and restart the gateway, or reinstall
genesis with browser support.
...
- Download and trace output paths are constrained to genesis temp roots:
  - traces: `/tmp/genesis` (fallback: `${os.tmpdir()}/genesis`)
  - downloads: `/tmp/genesis/downloads` (fallback: `${os.tmpdir()}/genesis/downloads`)
- Upload paths are constrained to an genesis temp uploads root:
  - uploads: `/tmp/genesis/uploads` (fallback: `${os.tmpdir()}/genesis/uploads`)
- `upload` can also set file inputs directly via `--input-ref` or `--element`.
- `snapshot`:
  - `--format ai` (default when Playwright is installed): returns an AI snapshot with numeric refs (`aria-ref="<n>"`).
  - `--format aria`: returns the accessibility tree (no refs; inspection only).
  - `--efficient` (or `--mode efficient`): compact role snapshot preset (interactive + compact + depth + lower maxChars).
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-genesis-managed-browser)).
