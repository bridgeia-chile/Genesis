...
ClawHub is the **public skill registry for genesis**. It is a free service: all skills are public, open, and visible to everyone for sharing and reuse. A skill is just a folder with a `SKILL.md` file (plus supporting text files). You can browse skills in the web app or use the CLI to search, install, update, and publish skills.

Site: [clawhub.ai](https://clawhub.ai)

## What ClawHub is

- A public registry for genesis skills.
...
4. Start a new genesis session so it picks up the new skill.
...
By default, the CLI installs skills into `./skills` under your current working directory. If a genesis workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). genesis loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.genesis/skills` or bundled skills, workspace skills take precedence.
