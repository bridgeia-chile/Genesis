---
summary: "Agent tool surface for genesis (browser, canvas, nodes, message, cron) replacing legacy `genesis-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `genesis-*` skills
title: "Tools"
---

# Tools (genesis)

genesis exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `genesis-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `genesis.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, genesis logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:genesis`: all built-in genesis tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`genesis node run`).
...
genesis tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `genesis configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated genesis-managed browser.
...
- Quick smoke: `genesis nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `genesis gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, genesis clamps `tools.sessions.visibility` to `tree`.
