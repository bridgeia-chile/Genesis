---
summary: "Direct `genesis agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `genesis agent` (direct agent runs)

`genesis agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `genesis message --target`).
...
genesis agent --to +15555550123 --message "Summon reply" --deliver
