---
summary: "Chrome extension: let genesis drive your existing Chrome tab"
read_when:
...
The genesis Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate genesis-managed Chrome profile.
...
genesis then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the genesis release (npm package) as static files. There is no separate “build” step.

After upgrading genesis:

- Re-run `genesis browser extension install` to refresh the installed files under your genesis state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

genesis ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `genesis_GATEWAY_TOKEN`)
...
- Open the tab you want genesis to control.
...
- `ON`: attached; genesis can drive that tab.
...
Debugging: `genesis sandbox explain`
...
`genesis browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `genesis browser extension install` first to copy the extension to a stable location under your genesis state directory.
...
- **This is not isolated** like the dedicated genesis-managed profile.
