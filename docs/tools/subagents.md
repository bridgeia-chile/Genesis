...
- For manual spawns, delivery is resilient:
  - genesis tries direct `agent` delivery first with a stable idempotency key.
...
2. genesis creates or binds a thread to that session target in the active channel.
