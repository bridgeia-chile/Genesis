...
- **gateway host** → `genesis` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.genesis/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.genesis/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `genesis approvals allowlist ...`).
- `genesis security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `genesis doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.genesis/exec-approvals.json` directly.

CLI: `genesis approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
