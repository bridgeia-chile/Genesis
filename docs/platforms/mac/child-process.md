...
service via the external `genesis` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`genesis_PROFILE`; legacy `com.genesis.*` is supported).
