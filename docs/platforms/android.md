...
- You can run the CLI (`genesis`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.bridgeia-chile/Genesis.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `genesis.internal.`) on the gateway host and publish `_genesis-gw._tcp` records.
...
1. Create `~/.genesis/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
genesis nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__genesis__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__genesis__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__genesis__/a2ui/`.
