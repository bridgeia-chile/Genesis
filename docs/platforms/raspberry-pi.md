---
summary: "genesis on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up genesis on a Raspberry Pi
  - Running genesis on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# genesis on Raspberry Pi

## Goal

Run a persistent, always-on genesis Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most genesis features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled genesis
...
```bash
# Check logs
journalctl -u genesis --no-pager -n 100

# Common fix: rebuild
cd ~/genesis  # if using hackable install
npm run build
sudo systemctl restart genesis
```
