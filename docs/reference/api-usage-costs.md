...
genesis features that can generate provider usage or paid API calls.
...
- `genesis status --usage` and `genesis channels list` show provider **usage windows**
...
genesis can pick up credentials from:
...
- `genesis status --usage`
- `genesis models status --json`
...
`genesis models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
