...
summary: 设置指南：在保持最新的同时保持你的 genesis 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.genesis/workspace`（工作区）+ `~/.bridgeia-chile/Genesis.json`（配置）。
...
- **配置：** `~/.bridgeia-chile/Genesis.json`（JSON/JSON5 格式）
- **工作区：** `~/.genesis/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm genesis setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **genesis.app**（菜单栏）。
...
- 运行 `genesis setup`，然后 `genesis channels login`，然后手动启动 Gateway 网关（`genesis gateway`）。
...
- **配对允许列表**：`~/.genesis/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.genesis/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.genesis/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.genesis/workspace` 和 `~/.genesis/` 保持为"你的东西"；不要将个人提示/配置放入 `genesis` 仓库。
