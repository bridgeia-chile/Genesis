---
read_when:
  - 你想让 genesis 在云 VPS 上 24/7 运行（而不是你的笔记本电脑）
  - 你想在自己的 VPS 上运行生产级、永久在线的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
  - 你在 Hetzner 或类似提供商上用 Docker 运行 genesis
summary: 在廉价的 Hetzner VPS（Docker）上 24/7 运行 genesis Gateway 网关，带持久状态和内置二进制文件
title: Hetzner
x-i18n:
...
# 在 Hetzner 上运行 genesis（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 Hetzner VPS 上运行持久的 genesis Gateway 网关，带持久状态、内置二进制文件和安全的重启行为。

如果你想要"约 $5 实现 genesis 24/7"，这是最简单可靠的设置。
...
- 在主机上持久化 `~/.genesis` + `~/.genesis/workspace`（重启/重建后保留）
...
chown -R 1000:1000 /root/.genesis/workspace
```
...
genesis 在 Docker 中运行，但 Docker 不是事实来源。
...
| WhatsApp 会话    | `/home/node/.genesis/`           | 主机卷挂载    | 保留二维码登录              |
