...
通过一键模板在 Railway 上部署 genesis，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/genesis` — 控制面板 UI
...
- `genesis_STATE_DIR=/data/.genesis`（推荐）
- `genesis_WORKSPACE_DIR=/data/workspace`（推荐）
- `genesis_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 genesis 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
