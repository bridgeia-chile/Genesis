...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.genesis/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.genesis/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.genesis`（或 `genesis_STATE_DIR`）
- 工作区：`~/.genesis/workspace`（或 `~/.genesis/workspace-<agentId>`）
- 智能体目录：`~/.genesis/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.genesis/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，genesis 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.genesis/workspace`（或当设置了 `genesis_PROFILE` 时为 `~/.genesis/workspace-<profile>`）。
- 状态默认为 `~/.genesis/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.genesis/credentials/whatsapp/personal
