...
LINE 通过 LINE Messaging API 连接到 genesis。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
genesis plugins install @genesis/line
```

本地检出（从 git 仓库运行时）：

```bash
genesis plugins install ./extensions/line
```
...
```bash
genesis pairing list line
genesis pairing approve line <CODE>
```
