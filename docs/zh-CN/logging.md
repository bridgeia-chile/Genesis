...
你可以在 `~/.bridgeia-chile/Genesis.json` 中覆盖此设置：
...
所有日志配置都在 `~/.bridgeia-chile/Genesis.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/bridgeia-chile/Genesis-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- genesis 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `genesis plugins enable diagnostics-otel` 启用插件。
...
- `genesis.tokens`（计数器，属性：`genesis.token`、`genesis.channel`、`genesis.provider`、`genesis.model`）
- `genesis.cost.usd`（计数器，属性：`genesis.channel`、`genesis.provider`、`genesis.model`）
- `genesis.run.duration_ms`（直方图，属性：`genesis.channel`、`genesis.provider`、`genesis.model`）
- `genesis.context.tokens`（直方图，属性：`genesis.context`、`genesis.channel`、`genesis.provider`、`genesis.model`）

消息流：

- `genesis.webhook.received`（计数器，属性：`genesis.channel`、`genesis.webhook`）
- `genesis.webhook.error`（计数器，属性：`genesis.channel`、`genesis.webhook`）
- `genesis.webhook.duration_ms`（直方图，属性：`genesis.channel`、`genesis.webhook`）
- `genesis.message.queued`（计数器，属性：`genesis.channel`、`genesis.source`）
- `genesis.message.processed`（计数器，属性：`genesis.channel`、`genesis.outcome`）
- `genesis.message.duration_ms`（直方图，属性：`genesis.channel`、`genesis.outcome`）

队列 + 会话：

- `genesis.queue.lane.enqueue`（计数器，属性：`genesis.lane`）
- `genesis.queue.lane.dequeue`（计数器，属性：`genesis.lane`）
- `genesis.queue.depth`（直方图，属性：`genesis.lane` 或 `genesis.channel=heartbeat`）
- `genesis.queue.wait_ms`（直方图，属性：`genesis.lane`）
- `genesis.session.state`（计数器，属性：`genesis.state`、`genesis.reason`）
- `genesis.session.stuck`（计数器，属性：`genesis.state`）
- `genesis.session.stuck_age_ms`（直方图，属性：`genesis.state`）
- `genesis.run.attempt`（计数器，属性：`genesis.attempt`）

### 导出的 span（名称 + 关键属性）

- `genesis.model.usage`
  - `genesis.channel`、`genesis.provider`、`genesis.model`
  - `genesis.sessionKey`、`genesis.sessionId`
...
- `genesis.webhook.error`
  - `genesis.channel`、`genesis.webhook`、`genesis.chatId`、`genesis.error`
- `genesis.message.processed`
  - `genesis.channel`、`genesis.outcome`、`genesis.chatId`、`genesis.messageId`、`genesis.sessionKey`、`genesis.sessionId`、`genesis.reason`
- `genesis.session.stuck`
  - `genesis.state`、`genesis.ageMs`、`genesis.queueDepth`、`genesis.sessionKey`、`genesis.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `genesis doctor`。
