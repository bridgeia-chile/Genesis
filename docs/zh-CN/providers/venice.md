---
read_when:
  - 你想在 genesis 中使用注重隐私的推理服务
  - 你需要 Venice AI 设置指导
summary: 在 genesis 中使用 Venice AI 注重隐私的模型
title: Venice AI
x-i18n:
...
```bash
genesis chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## 模型选择

设置完成后，genesis 会显示所有可用的 Venice 模型。根据你的需求选择：
...
## 通过 `genesis configure` 配置

1. 运行 `genesis configure`
2. 选择 **Model/auth**
3. 选择 **Venice AI**
...
当设置了 `VENICE_API_KEY` 时，genesis 会自动从 Venice API 发现模型。如果 API 不可达，则回退到静态目录。
...
```bash
# 使用默认私密模型
genesis chat --model venice/llama-3.3-70b

# 通过 Venice 使用 Claude（匿名化）
genesis chat --model venice/claude-opus-45

# 使用无审查模型
genesis chat --model venice/venice-uncensored

# 使用视觉模型处理图像
genesis chat --model venice/qwen3-vl-235b-a22b

# 使用编程模型
genesis chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
Venice 模型目录会动态更新。运行 `genesis models list` 查看当前可用的模型。部分模型可能暂时离线。
