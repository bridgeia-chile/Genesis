...
read_when:
  ...
  - 你想在 genesis 中通过 OpenRouter 运行模型
summary: 使用 OpenRouter 的统一 API 在 genesis 中访问多种模型
title: OpenRouter
x-i18n:
...
```bash
genesis onboard --auth-choice apiKey --token-provider openrouter --token "$OPENROUTER_API_KEY"
```
