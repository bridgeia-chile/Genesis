---
read_when:
  - 你想在 genesis 中使用 Z.AI / GLM 模型
  - 你需要简单的 ZAI_API_KEY 配置
summary: 在 genesis 中使用智谱 AI（GLM 模型）
title: Z.AI
x-i18n:
...
Z.AI 是 **GLM** 模型的 API 平台。它为 GLM 提供 REST API，并使用 API 密钥进行身份验证。请在 Z.AI 控制台中创建你的 API 密钥。genesis 通过 `zai` 提供商配合 Z.AI API 密钥使用。
...
# 或非交互式
genesis onboard --zai-api-key "$ZAI_API_KEY"
```
