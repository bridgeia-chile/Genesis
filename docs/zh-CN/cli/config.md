...
read_when:
  - 你想以非交互方式读取或编辑配置
summary: "`genesis config` 的 CLI 参考（获取/设置/取消设置配置值）"
title: config
x-i18n:
...
genesis config set gateway.port 19001 --json
genesis config set channels.whatsapp.groups '["*"]' --json
```
