...
- **Media understanding (audio)**: If audio understanding is enabled (or auto‑detected), genesis:
...
genesis auto-detects in this order and stops at the first working option:
...
When `requireMention: true` is set for a group chat, genesis now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, genesis performs a "preflight" transcription.
