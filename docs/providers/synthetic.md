---
summary: "Use Synthetic's Anthropic-compatible API in genesis"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. genesis registers it as the
...
Note: genesis's Anthropic client appends `/v1` to the base URL, so use
