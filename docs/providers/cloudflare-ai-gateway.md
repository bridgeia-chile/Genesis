...
read_when:
  - You want to use Cloudflare AI Gateway with genesis
...
If the Gateway runs as a daemon (launchd/systemd), make sure `CLOUDFLARE_AI_GATEWAY_API_KEY` is available to that process (for example, in `~/.genesis/.env` or via `env.shellEnv`).
