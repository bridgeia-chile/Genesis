---
summary: "Use MiniMax M2.1 in genesis"
read_when:
...
```bash
genesis plugins enable minimax-portal-auth  # skip if already loaded.
genesis gateway restart  # restart if gateway is already running
genesis onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/bridgeia-chile/Genesis/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `genesis.json`:
...
- Use `genesis models list` and `genesis models set minimax/MiniMax-M2.1` to switch.
...
- Running `genesis configure` and selecting **MiniMax M2.1**, or
