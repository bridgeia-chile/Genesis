---
summary: "Use Venice AI privacy-focused models in genesis"
read_when:
  - You want privacy-focused inference in genesis
...
```bash
genesis onboard --auth-choice venice-api-key
```
...
```bash
genesis agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, genesis shows all available Venice models. Pick based on your needs:
...
```bash
genesis models set venice/claude-opus-45
genesis models set venice/llama-3.3-70b
```

List all available models:

```bash
genesis models list | grep venice
```

## Configure via `genesis configure`

1. Run `genesis configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
genesis automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
genesis agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
genesis agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
genesis agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
genesis agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
genesis agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `genesis models list` to see currently available models. Some models may be temporarily offline.
