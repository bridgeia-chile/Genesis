...
import ai.genesis.android.ScreenCaptureRequester
...
class ScreenRecordManager(private val context: Context) {
  ...
  @Volatile private var permissionRequester: ai.genesis.android.PermissionRequester? = null

  fun attachScreenCaptureRequester(requester: ScreenCaptureRequester) {
    screenCaptureRequester = requester
  }

  fun attachPermissionRequester(requester: ai.genesis.android.PermissionRequester) {
  ...
  suspend fun record(paramsJson: String?): Payload =
    withContext(Dispatchers.Default) {
      ...
      val file = File.createTempFile("genesis-screen-", ".mp4")
