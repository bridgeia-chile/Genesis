...
class AppUpdateHandlerTest {
  ...
  fun parseAppUpdateRequest_acceptsHttpsWithMatchingHost() {
    val req =
      parseAppUpdateRequest(
        paramsJson =
          """{"url":"https://gw.example.com/releases/genesis.apk","sha256":"${"a".repeat(64)}"}""",
        connectedHost = "gw.example.com",
      )

    assertEquals("https://gw.example.com/releases/genesis.apk", req.url)
  ...
  fun parseAppUpdateRequest_rejectsNonHttps() {
    assertThrows(IllegalArgumentException::class.java) {
      parseAppUpdateRequest(
        paramsJson = """{"url":"http://gw.example.com/releases/genesis.apk","sha256":"${"a".repeat(64)}"}""",
        connectedHost = "gw.example.com",
      )
    }
  }
  ...
  fun parseAppUpdateRequest_rejectsHostMismatch() {
    assertThrows(IllegalArgumentException::class.java) {
      parseAppUpdateRequest(
        paramsJson = """{"url":"https://evil.example.com/releases/genesis.apk","sha256":"${"a".repeat(64)}"}""",
        connectedHost = "gw.example.com",
      )
    }
  }
  ...
  fun parseAppUpdateRequest_rejectsInvalidSha256() {
    assertThrows(IllegalArgumentException::class.java) {
      parseAppUpdateRequest(
        paramsJson = """{"url":"https://gw.example.com/releases/genesis.apk","sha256":"bad"}""",
        connectedHost = "gw.example.com",
      )
    }
  }
  ...
  fun sha256Hex_computesExpectedDigest() {
    val tmp = File.createTempFile("genesis-update-hash", ".bin")
    try {
