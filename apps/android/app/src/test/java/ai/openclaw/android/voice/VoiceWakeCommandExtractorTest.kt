package ai.genesis.android.voice
...
class VoiceWakeCommandExtractorTest {
  ...
  fun extractsCommandAfterTriggerWord() {
    val res = VoiceWakeCommandExtractor.extractCommand("Claude take a photo", listOf("genesis", "claude"))
    assertEquals("take a photo", res)
  }

  @Test
  fun extractsCommandWithPunctuation() {
    val res = VoiceWakeCommandExtractor.extractCommand("hey genesis, what's the weather?", listOf("genesis"))
