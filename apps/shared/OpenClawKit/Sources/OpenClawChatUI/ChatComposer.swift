...
@MainActor
struct genesisChatComposer: View {
    @Bindable var viewModel: genesisChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(genesisChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(genesisChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \genesisPendingAttachment.id)
                { (att: genesisPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            genesisPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(genesisChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(genesisChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(genesisChatTheme.subtleCard)
        .clipShape(Capsule())
    }
