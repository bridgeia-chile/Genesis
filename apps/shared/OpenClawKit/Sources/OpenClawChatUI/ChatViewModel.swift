...
private let chatUILogger = Logger(subsystem: "ai.genesis", category: "genesisChatUI")

@MainActor
@Observable
public final class genesisChatViewModel {
    public private(set) var messages: [genesisChatMessage] = []
    ...
    public var attachments: [genesisPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [genesisChatPendingToolCall] = []
    public private(set) var sessions: [genesisChatSessionEntry] = []
    private let transport: any genesisChatTransport
    ...
    private var pendingToolCallsById: [String: genesisChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any genesisChatTransport) {
    ...
    public var sessionChoices: [genesisChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [genesisChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: genesisPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [genesisChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: genesisChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: genesisChatMessage) -> genesisChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> genesisChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return genesisChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: genesisChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [genesisChatMessage],
        incoming: [genesisChatMessage]) -> [genesisChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [genesisChatMessage]) -> [genesisChatMessage] {
        var result: [genesisChatMessage] = []
    ...
    private static func dedupeKey(for message: genesisChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [genesisChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> genesisChatAttachmentPayload in
            genesisChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> genesisChatSessionEntry {
        genesisChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: genesisChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: genesisChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: genesisAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = genesisChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> genesisPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
