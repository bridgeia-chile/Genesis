...
private final class TestChatTransport: @unchecked Sendable, genesisChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [genesisChatHistoryPayload]
    private let sessionsResponses: [genesisChatSessionsListResponse]

    private let stream: AsyncStream<genesisChatTransportEvent>
    private let continuation: AsyncStream<genesisChatTransportEvent>.Continuation

    init(
        historyResponses: [genesisChatHistoryPayload],
        sessionsResponses: [genesisChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<genesisChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> genesisChatHistoryPayload {
        ...
        return self.historyResponses.last ?? genesisChatHistoryPayload(
        ...
        attachments _: [genesisChatAttachmentPayload]) async throws -> genesisChatSendResponse
    {
        ...
        return genesisChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> genesisChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? genesisChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: genesisChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = genesisChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = genesisChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { genesisChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = genesisChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = genesisChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = genesisChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { genesisChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
