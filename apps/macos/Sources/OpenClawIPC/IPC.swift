...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/genesis/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    return home
        .appendingPathComponent("Library/Application Support/genesis/control.sock")
        .path
}()
