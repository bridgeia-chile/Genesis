...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        let probeName = "_genesis-gw._tcp.\(domainTrimmed)"
        ...
        for raw in ptrLines {
            ...
            let suffix = "._genesis-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = genesisBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_genesis-gw._tcp.\(domainTrimmed)"
