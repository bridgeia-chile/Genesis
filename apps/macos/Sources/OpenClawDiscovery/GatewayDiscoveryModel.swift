...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.genesis", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in genesisBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: genesisBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.genesis.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = genesisBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = genesisBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = genesisBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (genesis)", with: "")
