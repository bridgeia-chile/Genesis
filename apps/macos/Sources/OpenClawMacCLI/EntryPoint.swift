...
struct genesisMacCLI {
    static func main() async {
        ...
        default:
            fputs("genesis-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      genesis-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      genesis-mac discover [--timeout <ms>] [--json] [--include-local]
      genesis-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      genesis-mac connect
      genesis-mac connect --url ws://127.0.0.1:18789 --json
      genesis-mac discover --timeout 3000 --json
      genesis-mac wizard --mode local
    """)
}
