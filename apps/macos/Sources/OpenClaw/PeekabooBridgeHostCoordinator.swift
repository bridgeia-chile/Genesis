...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.genesis", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: genesisPeekabooBridgeServices?
    private static var genesisSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("genesis", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = genesisPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.genesisSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.genesisSocketPath, privacy: .public)")
    }
...
@MainActor
private final class genesisPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.genesis.peekaboo")
