...
private let webChatSwiftLogger = Logger(subsystem: "ai.genesis", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: genesisChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> genesisChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(genesisChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> genesisChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                genesisGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<genesisChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any genesisChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = genesisChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: genesisChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<genesisChatView>) -> NSViewController
    {
