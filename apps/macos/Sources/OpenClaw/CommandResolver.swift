...
enum CommandResolver {
    private static let projectRootDefaultsKey = "genesis.gatewayProjectRootPath"
    private static let helperName = "genesis"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let genesisEntry = root.appendingPathComponent("genesis.mjs").path
        if FileManager().isReadableFile(atPath: genesisEntry) { return genesisEntry }
        let binEntry = root.appendingPathComponent("bin/genesis.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/genesis")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let genesisPaths = self.genesisManagedPaths(home: home)
        if !genesisPaths.isEmpty {
            extras.insert(contentsOf: genesisPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + genesisPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func genesisManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".genesis"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func genesisExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectgenesisExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("genesis.mjs").path,
            root.appendingPathComponent("bin/genesis.js").path,
        ]
    ...
    static func hasAnygenesisInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.genesisExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func genesisNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let genesisPath = self.projectgenesisExecutable(projectRoot: root) {
                return [genesisPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "genesis", subcommand] + extraArgs
            }
            if let genesisPath = self.genesisExecutable(searchPaths: searchPaths) {
                return [genesisPath, subcommand] + extraArgs
            }

            let missingEntry = """
            genesis entrypoint missing (looked for dist/index.js or genesis.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real genesis CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/genesis"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/genesis.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/genesis.mjs"
            node "$PRJ/genesis.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/genesis.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/genesis.js"
            node "$PRJ/bin/genesis.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent genesis"
          pnpm --silent genesis \(quotedArgs);
        else
          echo "genesis CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? genesisConfigFile.loadDict()
