...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (genesis token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: genesisOAuthStore.AnthropicOAuthStatus = genesisOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.genesis", category: "anthropic-oauth")
...
enum genesisOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let genesisOAuthDirEnv = "genesis_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "genesis OAuth token file not found"
            case .unreadableFile: "genesis OAuth token file not readable"
            case .invalidJSON: "genesis OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in genesis OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "genesis OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.genesisOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".genesis", isDirectory: true)
