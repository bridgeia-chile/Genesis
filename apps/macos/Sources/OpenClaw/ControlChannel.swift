...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: genesisProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.genesis", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: genesisKit.AnyCodable]()) {
                $0[$1.key] = genesisKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: genesisProtocol.AnyCodable?) -> [String: genesisProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: genesisProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: genesisKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: genesisProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
