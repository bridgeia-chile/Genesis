...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.genesis.mac`) even if the shipped
    ...
    {
        if environment["genesis_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "genesis.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "genesis.nixMode") { return true }
