...
enum DebugActions {
    private static let verboseDefaultsKey = "genesis.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "genesis.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = genesisPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "genesis", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = genesisPaths.configURL
        guard
