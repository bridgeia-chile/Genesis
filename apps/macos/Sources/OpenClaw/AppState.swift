...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = genesisConfigFile.loadDict()
    ...
    private func startConfigWatcher() {
        let configUrl = genesisConfigFile.url()
    ...
    private func syncGatewayConfigIfNeeded() {
        ...
        Task { @MainActor in
            ...
            var root = genesisConfigFile.loadDict()
            ...
            genesisConfigFile.saveDict(root)
        }
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/genesis"
        state.remoteCliPath = ""
        return state
    }
}
