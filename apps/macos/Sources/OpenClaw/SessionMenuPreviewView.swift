...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.genesis", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> genesisSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: genesisSessionPreviewEntry,
    ...
    private static func cache(payload: genesisSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: genesisChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [genesisKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [genesisKit.AnyCodable]) -> [genesisChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(genesisChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: genesisChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: genesisChatMessage) -> Bool {
    ...
    private static func toolNames(for message: genesisChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: genesisChatMessage) -> String? {
