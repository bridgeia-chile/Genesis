...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: genesisOAuthStore.AnthropicOAuthStatus = genesisOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: genesisOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = genesisOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = genesisOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try genesisOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. genesis can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: genesisOAuthStore.AnthropicOAuthStatus,
