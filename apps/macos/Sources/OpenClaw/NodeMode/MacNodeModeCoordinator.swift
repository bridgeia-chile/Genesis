...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.genesis", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: genesisNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [genesisCapability.canvas.rawValue, genesisCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(genesisCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if genesisLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(genesisCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            genesisSystemCommand.which.rawValue,
            genesisSystemCommand.run.rawValue,
            genesisSystemCommand.execApprovalsGet.rawValue,
            genesisSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(genesisCapability.camera.rawValue) {
        ...
        if capsSet.contains(genesisCapability.location.rawValue) {
