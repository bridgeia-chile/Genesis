...
enum genesisLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = genesisOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = genesisFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        genesisLogging.bootstrapIfNeeded()
        let label = genesisLogging.makeLabel(subsystem: subsystem, category: category)
...
struct genesisOSLogHandler: LogHandler {
...
struct genesisFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = genesisLogging.parseLabel(self.label)
