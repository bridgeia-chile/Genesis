...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "genesisCanvasA2UIAction"
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        let text = genesisCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = genesisCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in genesisKit (`genesisCanvasA2UIAction`).
}
