        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "genesis active",
                        subtitle: "Pause to stop the genesis gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start genesis after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep genesis visible in the Dock instead of menu-bar-only mode.",
                ...
                HStack {
                    Spacer()
                    Button("Quit genesis") { NSApp.terminate(nil) }
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/genesis.app/.../genesis", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for genesis logs in /tmp/genesis/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if let endpoint = GatewayDiscoveryHelpers.serviceEndpoint(for: gateway) {
            genesisConfigFile.setRemoteGatewayUrl(
                host: endpoint.host,
                port: endpoint.port)
        } else {
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/genesis"
        state.remoteCliPath = "/tmp/genesis"
