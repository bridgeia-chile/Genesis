...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.genesis/genesis.json. Prefer genesis_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.genesis/genesis.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.genesis/genesis.json. Restart the gateway to apply."
        }
