...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["genesis"]
        let text = "hey genesis"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey genesis do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["genesis"], minPostTriggerGap: 0.3)
