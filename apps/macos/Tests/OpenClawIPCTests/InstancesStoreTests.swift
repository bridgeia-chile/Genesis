...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: genesisProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: genesisProtocol.AnyCodable] = [
            "presence": .init([genesisProtocol.AnyCodable(entry)]),
        ]
        let payload = genesisProtocol.AnyCodable(payloadMap)
