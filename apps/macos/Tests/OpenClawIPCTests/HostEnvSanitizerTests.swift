...
struct HostEnvSanitizerTests {
    ...
    @Test func sanitizeShellWrapperAllowsOnlyExplicitOverrideKeys() {
        ...
        #expect(env["LANG"] == "C")
        #expect(env["LC_ALL"] == "C")
        #expect(env["genesis_TOKEN"] == nil)
        #expect(env["PS4"] == nil)
    }

    @Test func sanitizeNonShellWrapperKeepsRegularOverrides() {
        let env = HostEnvSanitizer.sanitize(overrides: ["genesis_TOKEN": "secret"])
        #expect(env["genesis_TOKEN"] == "secret")
    }
}
