# genesis iOS (Super Alpha)

NO TEST FLIGHT AVAILABLE AT THIS POINT

This iPhone app is super-alpha and internal-use only. It connects to an genesis Gateway as a `role: node`.
...
- `apps/ios/Sources/genesis.entitlements` sets `aps-environment` to `development`.
