...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.genesis.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to genesis", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to genesis gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to genesis.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "genesisShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "genesis is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "genesis Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("genesis-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
