...
final class RemindersService: RemindersServicing {
    func list(params: genesisRemindersListParams) async throws -> genesisRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[genesisReminderPayload], Error>) in
        ...
        return genesisRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: genesisRemindersAddParams) async throws -> genesisRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = genesisReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return genesisRemindersAddPayload(reminder: payload)
    }
