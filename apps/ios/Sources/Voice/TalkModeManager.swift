...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__genesis_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<genesisTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = genesisTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> genesisTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return genesisTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> genesisTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> genesisTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return genesisTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> genesisTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = genesisTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: genesisTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: genesisAgentEventPayload.self) else {
                continue
            }
