...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> genesisNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> genesisNetworkStatusPayload {
        let status: genesisNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> genesisNetworkStatusPayload {
        genesisNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
