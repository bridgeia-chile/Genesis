...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.genesis.gateway"
    private static let nodeService = "ai.genesis.node"
    private static let talkService = "ai.genesis.talk"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.genesis.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.genesis.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("genesis-gateway.log")
    }
