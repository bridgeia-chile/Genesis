...
final class MotionService: MotionServicing {
    func activities(params: genesisMotionActivityParams) async throws -> genesisMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[genesisMotionActivityEntry], Error>) in
        ...
        return genesisMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: genesisPedometerParams) async throws -> genesisPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<genesisPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = genesisPedometerPayload(
