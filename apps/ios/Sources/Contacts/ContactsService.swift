...
final class ContactsService: ContactsServicing {
    ...
    func search(params: genesisContactsSearchParams) async throws -> genesisContactsSearchPayload {
        ...
        return genesisContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: genesisContactsAddParams) async throws -> genesisContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return genesisContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return genesisContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> genesisContactPayload {
        genesisContactPayload(
