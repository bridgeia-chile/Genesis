...
@MainActor
final class genesisAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.genesis.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.genesis.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.genesis.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "genesis.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "genesis.watch.promptId"
    static let sessionKeyKey = "genesis.watch.sessionKey"
    static let actionPrimaryIDKey = "genesis.watch.action.primary.id"
    static let actionPrimaryLabelKey = "genesis.watch.action.primary.label"
    static let actionSecondaryIDKey = "genesis.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "genesis.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "genesis.watch.action.primary"
    static let actionSecondaryIdentifier = "genesis.watch.action.secondary"
    static let categoryPrefix = "genesis.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: genesisWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> genesisWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return genesisWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "genesis" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: genesisWatchAction,
        secondaryAction: genesisWatchAction?) -> [UNNotificationAction]
    {
...
extension genesisApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("genesis: installing uncaught exception handler")
