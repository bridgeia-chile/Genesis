...
struct IOSGatewayChatTransport: genesisChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.genesis", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> genesisChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(genesisChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> genesisChatHistoryPayload {
        ...
        return try JSONDecoder().decode(genesisChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [genesisChatAttachmentPayload]) async throws -> genesisChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [genesisChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(genesisChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(genesisGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: genesisGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: genesisChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: genesisAgentEventPayload.self)
                        {
