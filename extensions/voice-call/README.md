...
Official Voice Call plugin for **genesis**.
...
### Option A: install via genesis (recommended)
...
```bash
mkdir -p ~/.genesis/extensions
cp -R extensions/voice-call ~/.genesis/extensions/voice-call
cd ~/.genesis/extensions/voice-call && pnpm install
```
