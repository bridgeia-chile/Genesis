...
function resolvegenesisRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.genesis_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set genesis_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolvegenesisRoot(), "dist", "extensionAPI.js");
