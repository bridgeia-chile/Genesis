...
import type { genesisPluginApi, genesisPluginService } from "genesis/plugin-sdk";
...
function normalizeDenyList(cfg: genesisPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: genesisPluginApi["config"]): string[] {
...
export default function register(api: genesisPluginApi) {
  ...
  const timerService: genesisPluginService = {
