...
import { approveDevicePairing, listDevicePairing } from "genesis/plugin-sdk";
...
function resolveGatewayPort(cfg: genesisPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.genesis_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: genesisPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: genesisPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the genesis iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the genesis iOS app:",
            "",
            "```",
            qrAscii,
            "```",
