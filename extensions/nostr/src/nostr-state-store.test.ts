...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.genesis_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-nostr-"));
  process.env.genesis_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.genesis_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".genesis");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.genesis_STATE_DIR = previous;
    }
