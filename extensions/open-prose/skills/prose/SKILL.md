...
metadata: { "genesis": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == genesis `sessions_spawn`
- **File I/O** == genesis `read`/`write`
- **Remote fetch** == genesis `web_fetch` (or `exec` with curl when POST is required)
