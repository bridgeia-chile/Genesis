...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with genesis.
// When running from source (tests/dev), genesis internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { genesisPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: genesisPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via genesis.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-llm-task-"));
