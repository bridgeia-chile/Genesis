...
`genesis.invoke --each`) without adding new genesis code per workflow.
...
This extension depends on genesis internal modules (the embedded agent runner).
It is intended to ship as a **bundled** genesis extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.genesis/extensions` as a standalone plugin directory.
