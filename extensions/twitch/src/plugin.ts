/**
 * Twitch channel plugin for genesis.
 *
...
import { buildChannelConfigSchema } from "genesis/plugin-sdk";
 ...
 * for genesis. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: genesisConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: genesisConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: genesisConfig): boolean => {
