...
import type { genesisPluginApi, genesisPluginToolContext } from "../../../src/plugins/types.js";
import {
...
function fakeApi(overrides: Partial<genesisPluginApi> = {}): genesisPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<genesisPluginToolContext> = {}): genesisPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "genesis-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: genesisPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
