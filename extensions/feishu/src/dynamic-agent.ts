...
import type { genesisConfig, PluginRuntime } from "genesis/plugin-sdk";
import type { DynamicAgentCreationConfig } from "./types.js";

export type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: genesisConfig;
  agentId?: string;
};
...
export async function maybeCreateDynamicAgent(params: {
  ...
  if (existingAgent) {
    ...
    const updatedCfg: genesisConfig = {
      ...cfg,
      bindings: [
        ...existingBindings,
        {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.genesis/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.genesis/agents/{agentId}/agent";
