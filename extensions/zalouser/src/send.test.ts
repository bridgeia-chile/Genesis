...
describe("zalouser send helpers", () => {
  ...
  it("uses env profile fallback and builds link command", async () => {
    ...
    const result = await sendLinkZalouser("thread-5", " https://genesis.ai ", { isGroup: true });

    expect(mockRunZca).toHaveBeenCalledWith(
      ["msg", "link", "thread-5", "https://genesis.ai", "-g"],
      { profile: "env-profile" },
    );
  ...
  it("returns caught command errors", async () => {
    ...
    await expect(sendLinkZalouser("thread-6", "https://genesis.ai")).resolves.toEqual({
      ok: false,
