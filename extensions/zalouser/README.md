...
genesis extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
genesis channels login --channel zalouser              # Login via QR
...
```bash
# Text
genesis message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
genesis message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `genesis channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
genesis channels login --channel zalouser --account work
genesis message send --channel zalouser --account work --target <id> --message "Hello"
